...
/* Change log:
 ...
 As from V5.0.7:
  In L0_WriteRegister (legacy function for register access): Typo corrected in L0_WriteRegister. This only had an impact on register-based parts.

 As from V5.0.6:
  In L0_WriteRegister (legacy function for register access):
    Changing initial shift value. The previous code did not shift enough bits. This had no impact on the result, but it's cleaner now.
    (It would only change the written bits if provided an out of range register value)
...
int     L0_ReadBytes         (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  switch (i2c->connectionType) {
    ...
    case LINUX_KERNEL_I2C:
      ...
      if ( SDK2_I2cRead (i2c->SDK2_i2c_Chip, 0x00, pucDataBuffer, iNbBytes) != 0 ) {
        nbReadBytes = 0;
      } else {
        nbReadBytes = iNbBytes;
      }
    ...
    case CUSTOMER:
      ...
      */
      ...
      if ((i2c->mustReadWithoutStop == 0)
          ...
          nbReadBytes = iNbBytes;
      else if ((i2c->mustReadWithoutStop == 1)
            && (I2CReadIndex(i2c->user, i2c->address>>1, pucAddressBuffer, i2c->indexSize, pucDataBuffer, iNbBytes) == iNbBytes))
          nbReadBytes = iNbBytes;
      else
          nbReadBytes = 0;
#endif /* LINUX_KERNEL_DRIVER */
...
long    L0_WriteRegister     (L0_Context *i2c, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned char alone, long Value) {
  ...
  Value = (unsigned long)Value << (8*sizeof(unsigned long) - nbbit);
  Value = (unsigned long)Value >> (8*sizeof(unsigned long) - nbbit);
...
const char*   Si_I2C_TAG_TEXT      (void) { return (char *)"V5.0.7"; }
/************************************************************************************************************************
  L0_FastI2C  function
