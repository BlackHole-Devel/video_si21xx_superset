...
#include "Si2147_firmware_3_1_build_3.h"       /*  firmware file for  patch  */
    ...
    /* Load the Firmware */
     if (!(api->load_control & SKIP_LOADFIRMWARE))
     {
        ...
        if (api->rsp->part_info.romid == 0x50) {
           ...
           if ((return_code = Si2147_LoadFirmware_16(api, Si2147_FW_3_1b3, FIRMWARE_LINES_3_1b3)) != NO_Si2147_ERROR) {
...
/************************************************************************************************************************
  ...
  Parameter:  tuners, a pointer to a table of L1 Si2147 Contexts
  Parameter:  tuner_count, the number of tuners in the table
  Returns:    Si2147/I2C transaction error code, NO_Si2147_ERROR if successful
************************************************************************************************************************/
int Si2147_PowerUpUsingBroadcastI2C    (L1_Si2147_Context *tuners[], int tuner_count )
{
    int return_code;
    int t;
    #define Si2147_TUNER_BROADCAST_INDEX 0
  ...
  for (t = 0; t < tuner_count; t++)
  {
      ...
      /* Si2147_PowerUpWithPatch will return right after Si2147_L1_PART_INFO, because SKIP_LOADFIRMWARE and SKIP_STARTFIRMWARE are set */
      if ( (return_code = Si2147_PowerUpWithPatch(tuners[t])) != NO_Si2147_ERROR) {
        SiTRACE ("Tuner %d Si2147_PowerUpWithPatch error 0x%02x\n", return_code);
        return return_code;
      }
       if ( (return_code = Si2147_L1_CONFIG_I2C(tuners[t], Si2147_CONFIG_I2C_CMD_SUBCODE_CODE, Si2147_CONFIG_I2C_CMD_I2C_BROADCAST_ENABLED)) != NO_Si2147_ERROR )
       {
  ...
  logged_i2c_address = tuners[Si2147_TUNER_BROADCAST_INDEX]->i2c->address;
  tuners[Si2147_TUNER_BROADCAST_INDEX]->i2c->address = Si2147_BROADCAST_ADDRESS;
  /* set the load_control flag to SKIP_POWERUP so the firmware is downloaded and started on all tuners only */
  tuners[Si2147_TUNER_BROADCAST_INDEX]->load_control = SKIP_POWERUP;

  /* Si2147_PowerUpWithPatch will now broadcast the tuner fw and return
      when all is completed, because load_fw is now '1'                               */
  if ( (return_code = Si2147_PowerUpWithPatch(tuners[Si2147_TUNER_BROADCAST_INDEX])) != NO_Si2147_ERROR) {
    SiTRACE("Tuner %d Si2147_PowerUpWithPatch error 0x%02x\n", return_code);
    return return_code;
  }
  ...
  /* At this stage, all tuners have received the patch, and have been issued
     ...
     if ((return_code = Si2147_Configure    (tuners[Si2147_TUNER_BROADCAST_INDEX])) != NO_Si2147_ERROR)
     {
         return return_code;
     }

  /* Return the broadcast tuner address to its 'normal' value                         */
  tuners[Si2147_TUNER_BROADCAST_INDEX]->i2c->address = logged_i2c_address;

  for (t = 0; t < tuner_count; t++)
  {
    if ( (return_code = Si2147_L1_CONFIG_I2C(tuners[t], Si2147_CONFIG_I2C_CMD_SUBCODE_CODE, Si2147_CONFIG_I2C_CMD_I2C_BROADCAST_DISABLED)) != NO_Si2147_ERROR )
     {
      SiTRACE("Tuner %d L1_CONFIG_I2C error 0x%02x\n", t, return_code);
         return return_code;
     }
  ...
  {
    /* Set Properties startup configuration         */
    Si2147_storePropertiesDefaults (tuners[t]->propShadow);
    /* Edit the Si2147_storeUserProperties if you have any properties settings different from the standard defaults */
    Si2147_storeUserProperties     (tuners[t]->prop);
 ...
 /* Check CTS for all tuners */
    if ( (return_code = Si2147_pollForCTS(tuners[t])) != NO_Si2147_ERROR ) {
