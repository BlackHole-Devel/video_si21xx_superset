 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2141 Layer 3 API


   L3 sample application
   FILE: Si2141_L3_Test.c
   Supported IC : Si2141
   ...
   Date: June 14 2019
****************************************************************************************/
#include "Si2141_L3_Test.h"

/* define TUNER_ONLY if using a Si2141 tuner without demod                                                    */
...
/* define Si2141_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2141_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2141_COMMAND_LINE_APPLICATION */
...
/* If using a Silabs Demod and > 1 tuner then undef both flags and define Si2141_DEMOD_WRAPPER_VCO (see Si2141_L2_API.c) */
#if FRONT_END_COUNT > 1
#define Si2141_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2141_USE_PRETUNE_VCO_BLOCKING_METHOD
#else
#undef Si2141_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2141_USE_PRETUNE_VCO_BLOCKING_METHOD
#endif
...
L1_Si2141_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2141_Context         *tuners[FRONT_END_COUNT];
L1_Si2141_Context         *Si2141;
L1_Si2141_Context         *front_end;
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2141_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2141_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2141_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2141_UserInputString function
...
************************************************************************************************************************/
int  Si2141_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2141_trace                          (L1_Si2141_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2141_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2141 EVB
...
/************************************************************************************************************************
  NAME: Si2141_DisplayStatusByte
  DESCRIPTION:Read Si2141 STATUS byte and display status bits
  Parameter:  Pointer to Si2141 Context (I2C address)
  Returns:    Si2141/I2C transaction error code
************************************************************************************************************************/
int  Si2141_DisplayStatusByte              (L1_Si2141_Context *api)
{
    int error_code;

    error_code = Si2141_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2141_GetRevision
  DESCRIPTION:  Execute Si2141 GET_REV function and display response fields
  Parameter:  Pointer to Si2141 Context (I2C address)
...
int  Si2141_GetRevision                    (L1_Si2141_Context *api)
{
    ...
    if (Si2141_L1_GET_REV(api) != NO_Si2141_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2141_SENDING_COMMAND;
    }
    Si2141_L1_GetCommandResponseString(api, Si2141_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2141_TunerStatus
...
int Si2141_TunerStatus (L1_Si2141_Context *Si2141)
{
    ...
    if (Si2141_L1_TUNER_STATUS(Si2141) != NO_Si2141_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2141_SENDING_COMMAND;
    }
  Si2141_L1_GetCommandResponseString(Si2141, Si2141_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2141_GetUserFrequency
...
----------------------------------------------------------------------------\n\
       This is a demonstration application used to illustrate how to use    \n\
            the Si2141 API delivered by Silicon Laboratories                \n\
                                                                            \n\
...
");
  ...
  printf("\
InitAndConfig  : Initialize and Configure the Si2141\n\
Status         : Read Si2141 STATUS\n\
...
************************************************************************************************************************/
int Si2141_demoLoop (L1_Si2141_Context *Si2141, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
    freq = 0;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2141_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2141_trace (Si2141); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2141_Init(Si2141) == 0 ) {
           Si2141_Configure(Si2141);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2141 at 0x%02x!\n", Si2141->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2141_DisplayStatusByte     (Si2141); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2141_GetRevision           (Si2141); }
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2141_LoadVideofilter(Si2141,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2141_setupDTVProperties(Si2141);
         printf("Sending Properties to Si2141\n");
         Si2141_downloadDTVProperties(Si2141);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2141_setupTUNERProperties(Si2141);
         printf("Sending Properties to Si2141\n");
         Si2141_downloadTUNERProperties(Si2141);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2141_setupCOMMONProperties(Si2141);
         printf("Sending Properties to Si2141\n");
         Si2141_downloadCOMMONProperties(Si2141);
         printf("Done \n");
    }
   else if (stricmp(entry, "DTVTune"       )==0)
  {
        printf("DTV Center Frequency (in Hz)? ");
        if ((errcode=Si2141_GetUserFrequency(&freq)) != NO_Si2141_ERROR)
           return errcode;
        Si2141_setupProperty(Si2141,Si2141_PropertyIndex(Si2141, "DTV_MODE"));
    #ifdef Si2141_USE_PRETUNE_VCO_BLOCKING_METHOD
         if ((errcode =  Si2141_L2_VCO_Blocking_PreTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2141_ERROR)
         {
             return errcode;
         }
    #endif // Si2141_USE_PRETUNE_VCO_BLOCKING_METHOD

    errcode=Si2141_DTVTune (Si2141, freq, Si2141->prop->dtv_mode.bw, Si2141->prop->dtv_mode.modulation, Si2141->prop->dtv_mode.invert_spectrum);
    if ((errcode != NO_Si2141_ERROR ) && (errcode != ERROR_Si2141_xTVINT_TIMEOUT))
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2141_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

    #ifdef Si2141_USE_POSTTUNE_VCO_BLOCKING_METHOD
          if ((errcode =  Si2141_L2_VCO_Blocking_PostTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2141_ERROR)
          {
              return errcode;
          }
    #endif // Si2141_USE_POSTTUNE_VCO_BLOCKING_METHOD


  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2141_TunerStatus(Si2141);}
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2141_XoutOn(Si2141);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2141_XoutOff(Si2141);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2141_Powerdown(Si2141);}
    else if (stricmp(entry, "Standby"           )==0) { Si2141_Standby(Si2141);}
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2141_AGC_Override(Si2141, Si2141_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2141_AGC_Override(Si2141, Si2141_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2141_AGC_Override(Si2141, Si2141_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}
    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2141_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2141_help();}
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2141_L1_API_Init(front_end, tuner_addr[fe]);
      if (fe==0)
      {
          front_end->cmd->power_up.clock_mode =  Si2141_POWER_UP_CMD_CLOCK_MODE_XTAL;
          front_end->cmd->power_up.en_xout    =  Si2141_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2141_CONFIG_CLOCKS_CMD_CLOCK_MODE_XTAL;
      }
      else
      {
          front_end->cmd->power_up.clock_mode = Si2141_POWER_UP_CMD_CLOCK_MODE_EXTCLK;
          front_end->cmd->power_up.en_xout    = Si2141_POWER_UP_CMD_EN_XOUT_DIS_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2141_CONFIG_CLOCKS_CMD_CLOCK_MODE_EXTCLK;
      }

      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2141_configure_i2c_communication  (front_end);

    }
 /* If using 2 tuners (or more) startup both tuners using CONFIG_I2C command to broadcast firmware download */
#if FRONT_END_COUNT > 1

      if (Si2141_PowerUpUsingBroadcastI2C(tuners,FRONT_END_COUNT) != NO_Si2141_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2141's!\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }

#else /* use standard init for a single tuner */
      if (Si2141_Init(&(FrontEnd_Table[0])) != NO_Si2141_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2141's!\n");
...
#ifdef    Si2141_COMMAND_LINE_APPLICATION
    while (Si2141_demoLoop(front_end,""));
#endif /* Si2141_COMMAND_LINE_APPLICATION */
