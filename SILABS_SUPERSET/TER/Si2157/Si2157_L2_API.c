...
#include "Si2157_firmware_3_1_build_3.h" /*  firmware patch file */
    ...
    /* Load the Firmware */
     if (!(api->load_control & SKIP_LOADFIRMWARE))
     {
        ...
        if (api->rsp->part_info.romid == 0x50) {
           ...
           if ((return_code = Si2157_LoadFirmware_16(api, Si2157_FW_3_1b3, FIRMWARE_LINES_3_1b3)) != NO_Si2157_ERROR) {
...
/************************************************************************************************************************
  ...
  Parameter:  tuners, a pointer to a table of L1 Si2157 Contexts
...
int Si2157_PowerUpUsingBroadcastI2C    (L1_Si2157_Context *tuners[], int tuner_count )
{
    int return_code;
    int t;
    #define Si2157_TUNER_BROADCAST_INDEX 0
  ...
  for (t = 0; t < tuner_count; t++)
  {
      ...
      /* Si2157_PowerUpWithPatch will return right after Si2157_L1_PART_INFO, because SKIP_LOADFIRMWARE and SKIP_STARTFIRMWARE are set */
      if ( (return_code = Si2157_PowerUpWithPatch(tuners[t])) != NO_Si2157_ERROR) {
        SiTRACE ("Tuner %d Si2157_PowerUpWithPatch error 0x%02x\n", return_code);
        return return_code;
      }
       if ( (return_code = Si2157_L1_CONFIG_I2C(tuners[t], Si2157_CONFIG_I2C_CMD_SUBCODE_CODE, Si2157_CONFIG_I2C_CMD_I2C_BROADCAST_ENABLED)) != NO_Si2157_ERROR )
       {
  ...
  logged_i2c_address = tuners[Si2157_TUNER_BROADCAST_INDEX]->i2c->address;
  tuners[Si2157_TUNER_BROADCAST_INDEX]->i2c->address = Si2157_BROADCAST_ADDRESS;
  /* set the load_control flag to SKIP_POWERUP so the firmware is downloaded and started on all tuners only */
  tuners[Si2157_TUNER_BROADCAST_INDEX]->load_control = SKIP_POWERUP;

  /* Si2157_PowerUpWithPatch will now broadcast the tuner fw and return
      when all is completed, because load_fw is now '1'                               */
  if ( (return_code = Si2157_PowerUpWithPatch(tuners[Si2157_TUNER_BROADCAST_INDEX])) != NO_Si2157_ERROR) {
    SiTRACE("Tuner %d Si2157_PowerUpWithPatch error 0x%02x\n", return_code);
    return return_code;
  }
  ...
  /* At this stage, all tuners have received the patch, and have been issued
     ...
     if ((return_code = Si2157_Configure    (tuners[Si2157_TUNER_BROADCAST_INDEX])) != NO_Si2157_ERROR)
     {
         return return_code;
     }

  /* Return the broadcast tuner address to its 'normal' value                         */
  tuners[Si2157_TUNER_BROADCAST_INDEX]->i2c->address = logged_i2c_address;

  for (t = 0; t < tuner_count; t++)
  {
    if ( (return_code = Si2157_L1_CONFIG_I2C(tuners[t], Si2157_CONFIG_I2C_CMD_SUBCODE_CODE, Si2157_CONFIG_I2C_CMD_I2C_BROADCAST_DISABLED)) != NO_Si2157_ERROR )
     {
      SiTRACE("Tuner %d L1_CONFIG_I2C error 0x%02x\n", t, return_code);
         return return_code;
     }
  ...
  {
    /* Set Properties startup configuration         */
    Si2157_storePropertiesDefaults (tuners[t]->propShadow);
    /* Edit the Si2157_storeUserProperties if you have any properties settings different from the standard defaults */
    Si2157_storeUserProperties     (tuners[t]->prop);
 ...
 /* Check CTS for all tuners */
    if ( (return_code = Si2157_pollForCTS(tuners[t])) != NO_Si2157_ERROR ) {
