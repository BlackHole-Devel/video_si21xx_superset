...
/***********************************************************************************************************************
  Si2148_pollForResetState function
  Use:        ResetState checking function
              Used to check the status byte for the reset code (0xFE).
  Comments:   Only required for the Si2148_L1_POWER_DOWN_HW Command

  Returns:   0 if the Status byte is = 0xFE, nonzero otherwise
 ***********************************************************************************************************************/
unsigned char Si2148_pollForResetState (L1_Si2148_Context *api)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2148_pollForResetState ERROR reading byte 0!\n");
  ...
  SiTRACE("Si2148_pollForResetState ERROR status Timeout!\n");
                                         ...
                                         unsigned char   reserved8)
{
    ...
    error_code = Si2148_pollForResetState(api);
    if (error_code) {
