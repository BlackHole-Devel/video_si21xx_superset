 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2148 Layer 3 API


   L3 sample application
   FILE: Si2148_L3_Test.c
   Supported IC : Si2148
   ...
   Date: March 12 2014
****************************************************************************************/
#include "Si2148_L3_Test.h"

/* define TUNER_ONLY if using a Si2148 tuner without demod                                                    */
...
/* define Si2148_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2148_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2148_COMMAND_LINE_APPLICATION */
...
L1_Si2148_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2148_Context         *Si2148;
L1_Si2148_Context         *front_end;
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2148_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2148_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2148_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2148_UserInputString function
...
************************************************************************************************************************/
int  Si2148_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2148_trace                          (L1_Si2148_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2148_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2148 EVB
...
/************************************************************************************************************************
  NAME: Si2148_DisplayStatusByte
  DESCRIPTION:Read Si2148 STATUS byte and display status bits
  Parameter:  Si2148 Context (I2C address)
  Returns:    Si2148/I2C transaction error code
************************************************************************************************************************/
int  Si2148_DisplayStatusByte              (L1_Si2148_Context *api)
{
    int error_code;

    error_code = Si2148_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2148_GetRevision
  DESCRIPTION:  Execute Si2148 GET_REV function and display response fields
  Parameter:  Si2148 Context (I2C address)
...
int  Si2148_GetRevision                    (L1_Si2148_Context *api)
{
    ...
    if (Si2148_L1_GET_REV(api) != NO_Si2148_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2148_SENDING_COMMAND;
    }
    Si2148_L1_GetCommandResponseString(api, Si2148_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  Si2148_DTVStatus function
...
int Si2148_DTVStatus (L1_Si2148_Context *Si2148)
{
    ...
    if (Si2148_L1_DTV_STATUS(Si2148, Si2148_DTV_STATUS_CMD_INTACK_CLEAR) != 0) { /* execute DTV_STATUS and always read response */
        printf("Error reading DTV_STATUS\n");
        return ERROR_Si2148_SENDING_COMMAND;
    }
  Si2148_L1_GetCommandResponseString(Si2148, Si2148_DTV_STATUS_CMD_CODE,"\n", message);                  /* display results */
  printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2148_TunerStatus
...
int Si2148_TunerStatus (L1_Si2148_Context *Si2148)
{
    ...
    if (Si2148_L1_TUNER_STATUS(Si2148, Si2148_TUNER_STATUS_CMD_INTACK_CLEAR) != NO_Si2148_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2148_SENDING_COMMAND;
    }
  Si2148_L1_GetCommandResponseString(Si2148, Si2148_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2148_GetUserFrequency
...
/************************************************************************************************************************
  NAME: Si2148_menu
  DESCRIPTION: Si2148 user menu function
...
");
 ...
 printf("\
InitAndConfig  : Initialize and Configure the Si2148\n\
Status         : Read Si2148 STATUS\n\
...
************************************************************************************************************************/
int Si2148_demoLoop (L1_Si2148_Context *Si2148, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
    freq = 0;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2148_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2148_trace (Si2148); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2148_Init(Si2148) == 0 ) {
           Si2148_Configure(Si2148);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2148 at 0x%02x!\n", Si2148->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2148_DisplayStatusByte     (Si2148); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2148_GetRevision           (Si2148); }
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2148_setupDTVProperties(Si2148);
         printf("Sending Properties to Si2148\n");
         Si2148_downloadDTVProperties(Si2148);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2148_setupTUNERProperties(Si2148);
         printf("Sending Properties to Si2148\n");
         Si2148_downloadTUNERProperties(Si2148);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2148_setupCOMMONProperties(Si2148);
         printf("Sending Properties to Si2148\n");
         Si2148_downloadCOMMONProperties(Si2148);
         printf("Done \n");
    }
  else if (stricmp(entry, "DTVTune"       )==0)
  {
        printf("DTV Center Frequency (in Hz)? ");
        if ((errcode=Si2148_GetUserFrequency(&freq)) != NO_Si2148_ERROR)
           return errcode;
        Si2148_setupProperty(Si2148,Si2148_PropertyIndex(Si2148, "DTV_MODE"));

    if ((errcode=Si2148_DTVTune (Si2148, freq, Si2148->prop->dtv_mode.bw, Si2148->prop->dtv_mode.modulation, Si2148->prop->dtv_mode.invert_spectrum)) != NO_Si2148_ERROR)
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2148_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2148_TunerStatus(Si2148);}
    else if (stricmp(entry, "DTVStatus"     )==0) { Si2148_DTVStatus(Si2148);     }
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2148_XoutOn(Si2148);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2148_XoutOff(Si2148);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2148_Powerdown(Si2148);}
    else if (stricmp(entry, "Standby"           )==0) { Si2148_Standby(Si2148);}
    else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2148_AGC_Override(Si2148, Si2148_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2148_AGC_Override(Si2148, Si2148_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2148_AGC_Override(Si2148, Si2148_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}

    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2148_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2148_help();}
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2148_L1_API_Init(front_end, Si2148_BASE_ADDRESS+(2*fe));
      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2148_configure_i2c_communication  (front_end);
      if (Si2148_Init(front_end) != 0 ) {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2148 at 0x%02x!\n",Si2148_BASE_ADDRESS+(2*fe) );
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }
    }
...
#ifdef    Si2148_COMMAND_LINE_APPLICATION
    while (Si2148_demoLoop(front_end,""));
#endif /* Si2148_COMMAND_LINE_APPLICATION */
