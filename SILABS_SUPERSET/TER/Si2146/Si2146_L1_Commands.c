...
/***********************************************************************************************************************
  Si2146_CurrentResponseStatus function
  Use:        status checking function
              Used to fill the Si2146_COMMON_REPLY_struct members with the ptDataBuffer byte's bits
...
/* _status_code_insertion_point */
  return (api->status->err ? ERROR_Si2146_ERR : NO_Si2146_ERROR);
}

/***********************************************************************************************************************
  Si2146_pollForCTS function
...
unsigned char Si2146_pollForCTS (L1_Si2146_Context *api)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2146_pollForCTS ERROR reading byte 0!\n");
      return ERROR_Si2146_POLLING_CTS;
    }
  ...
  SiTRACE("Si2146_pollForCTS ERROR CTS Timeout!\n");
  return ERROR_Si2146_CTS_TIMEOUT;
}

/***********************************************************************************************************************
  Si2146_pollForResponse function
...
unsigned char Si2146_pollForResponse (L1_Si2146_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if ((unsigned int)L0_ReadCommandBytes(api->i2c, nbBytes, pByteBuffer) != nbBytes) {
      SiTRACE("Si2146_pollForResponse ERROR reading byte 0!\n");
  ...
  SiTRACE("Si2146_pollForResponse ERROR CTS Timeout!\n");
  return ERROR_Si2146_CTS_TIMEOUT;
}
                                                   ...
                                                   unsigned char   force_top_gain)
{
    ...
    SiTRACE("Si2146 AGC_OVERRIDE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((force_max_gain > Si2146_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_MAX_GAIN %d ", force_max_gain );
    if ((force_top_gain > Si2146_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_TOP_GAIN %d ", force_top_gain );
    if (error_code) {
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_AGC_OVERRIDE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( force_max_gain & Si2146_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MASK ) << Si2146_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_LSB|
                                         ( force_top_gain & Si2146_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MASK ) << Si2146_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_LSB);
                                         ...
                                         unsigned char   mode)
{
    ...
    SiTRACE("Si2146 ATV_RESTART ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((mode > Si2146_ATV_RESTART_CMD_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("MODE %d ", mode );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_ATV_RESTART_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( mode & Si2146_ATV_RESTART_CMD_MODE_MASK ) << Si2146_ATV_RESTART_CMD_MODE_LSB);
                                         ...
                                         unsigned char   intack)
{
    ...
    SiTRACE("Si2146 ATV_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2146_ATV_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_ATV_STATUS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( intack & Si2146_ATV_STATUS_CMD_INTACK_MASK ) << Si2146_ATV_STATUS_CMD_INTACK_LSB);
    ...
    error_code = Si2146_pollForResponse(api, 11, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling ATV_STATUS response\n");
      return error_code;
    }

    api->rsp->atv_status.chlint           =   (( ( (rspByteBuffer[1]  )) >> Si2146_ATV_STATUS_RESPONSE_CHLINT_LSB           ) & Si2146_ATV_STATUS_RESPONSE_CHLINT_MASK           );
    api->rsp->atv_status.pclint           =   (( ( (rspByteBuffer[1]  )) >> Si2146_ATV_STATUS_RESPONSE_PCLINT_LSB           ) & Si2146_ATV_STATUS_RESPONSE_PCLINT_MASK           );
    api->rsp->atv_status.dlint            =   (( ( (rspByteBuffer[1]  )) >> Si2146_ATV_STATUS_RESPONSE_DLINT_LSB            ) & Si2146_ATV_STATUS_RESPONSE_DLINT_MASK            );
    api->rsp->atv_status.snrlint          =   (( ( (rspByteBuffer[1]  )) >> Si2146_ATV_STATUS_RESPONSE_SNRLINT_LSB          ) & Si2146_ATV_STATUS_RESPONSE_SNRLINT_MASK          );
    api->rsp->atv_status.snrhint          =   (( ( (rspByteBuffer[1]  )) >> Si2146_ATV_STATUS_RESPONSE_SNRHINT_LSB          ) & Si2146_ATV_STATUS_RESPONSE_SNRHINT_MASK          );
    api->rsp->atv_status.chl              =   (( ( (rspByteBuffer[2]  )) >> Si2146_ATV_STATUS_RESPONSE_CHL_LSB              ) & Si2146_ATV_STATUS_RESPONSE_CHL_MASK              );
    api->rsp->atv_status.pcl              =   (( ( (rspByteBuffer[2]  )) >> Si2146_ATV_STATUS_RESPONSE_PCL_LSB              ) & Si2146_ATV_STATUS_RESPONSE_PCL_MASK              );
    api->rsp->atv_status.dl               =   (( ( (rspByteBuffer[2]  )) >> Si2146_ATV_STATUS_RESPONSE_DL_LSB               ) & Si2146_ATV_STATUS_RESPONSE_DL_MASK               );
    api->rsp->atv_status.snrl             =   (( ( (rspByteBuffer[2]  )) >> Si2146_ATV_STATUS_RESPONSE_SNRL_LSB             ) & Si2146_ATV_STATUS_RESPONSE_SNRL_MASK             );
    api->rsp->atv_status.snrh             =   (( ( (rspByteBuffer[2]  )) >> Si2146_ATV_STATUS_RESPONSE_SNRH_LSB             ) & Si2146_ATV_STATUS_RESPONSE_SNRH_MASK             );
    api->rsp->atv_status.video_snr        =   (( ( (rspByteBuffer[3]  )) >> Si2146_ATV_STATUS_RESPONSE_VIDEO_SNR_LSB        ) & Si2146_ATV_STATUS_RESPONSE_VIDEO_SNR_MASK        );
    api->rsp->atv_status.afc_freq         = (((( ( (rspByteBuffer[4]  ) | (rspByteBuffer[5]  << 8 )) >> Si2146_ATV_STATUS_RESPONSE_AFC_FREQ_LSB         ) & Si2146_ATV_STATUS_RESPONSE_AFC_FREQ_MASK) <<Si2146_ATV_STATUS_RESPONSE_AFC_FREQ_SHIFT ) >>Si2146_ATV_STATUS_RESPONSE_AFC_FREQ_SHIFT         );
    api->rsp->atv_status.video_sc_spacing = (((( ( (rspByteBuffer[6]  ) | (rspByteBuffer[7]  << 8 )) >> Si2146_ATV_STATUS_RESPONSE_VIDEO_SC_SPACING_LSB ) & Si2146_ATV_STATUS_RESPONSE_VIDEO_SC_SPACING_MASK) <<Si2146_ATV_STATUS_RESPONSE_VIDEO_SC_SPACING_SHIFT ) >>Si2146_ATV_STATUS_RESPONSE_VIDEO_SC_SPACING_SHIFT );
    api->rsp->atv_status.video_sys        =   (( ( (rspByteBuffer[8]  )) >> Si2146_ATV_STATUS_RESPONSE_VIDEO_SYS_LSB        ) & Si2146_ATV_STATUS_RESPONSE_VIDEO_SYS_MASK        );
    api->rsp->atv_status.color            =   (( ( (rspByteBuffer[8]  )) >> Si2146_ATV_STATUS_RESPONSE_COLOR_LSB            ) & Si2146_ATV_STATUS_RESPONSE_COLOR_MASK            );
    api->rsp->atv_status.trans            =   (( ( (rspByteBuffer[8]  )) >> Si2146_ATV_STATUS_RESPONSE_TRANS_LSB            ) & Si2146_ATV_STATUS_RESPONSE_TRANS_MASK            );
    api->rsp->atv_status.audio_sys        =   (( ( (rspByteBuffer[9]  )) >> Si2146_ATV_STATUS_RESPONSE_AUDIO_SYS_LSB        ) & Si2146_ATV_STATUS_RESPONSE_AUDIO_SYS_MASK        );
    api->rsp->atv_status.audio_demod_mode =   (( ( (rspByteBuffer[9]  )) >> Si2146_ATV_STATUS_RESPONSE_AUDIO_DEMOD_MODE_LSB ) & Si2146_ATV_STATUS_RESPONSE_AUDIO_DEMOD_MODE_MASK );
    api->rsp->atv_status.audio_chan_bw    =   (( ( (rspByteBuffer[10] )) >> Si2146_ATV_STATUS_RESPONSE_AUDIO_CHAN_BW_LSB    ) & Si2146_ATV_STATUS_RESPONSE_AUDIO_CHAN_BW_MASK    );
                                                   ...
                                                   unsigned char   xout_mode)
{
    ...
    SiTRACE("Si2146 CONFIG_PINS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((gpio1_mode > Si2146_CONFIG_PINS_CMD_GPIO1_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_MODE %d ", gpio1_mode );
    if ((gpio1_read > Si2146_CONFIG_PINS_CMD_GPIO1_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_READ %d ", gpio1_read );
    if ((gpio2_mode > Si2146_CONFIG_PINS_CMD_GPIO2_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_MODE %d ", gpio2_mode );
    if ((gpio2_read > Si2146_CONFIG_PINS_CMD_GPIO2_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_READ %d ", gpio2_read );
    if ((gpio3_mode > Si2146_CONFIG_PINS_CMD_GPIO3_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO3_MODE %d ", gpio3_mode );
    if ((gpio3_read > Si2146_CONFIG_PINS_CMD_GPIO3_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO3_READ %d ", gpio3_read );
    if ((bclk1_mode > Si2146_CONFIG_PINS_CMD_BCLK1_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("BCLK1_MODE %d ", bclk1_mode );
    if ((bclk1_read > Si2146_CONFIG_PINS_CMD_BCLK1_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("BCLK1_READ %d ", bclk1_read );
    if ((xout_mode  > Si2146_CONFIG_PINS_CMD_XOUT_MODE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("XOUT_MODE %d " , xout_mode  );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
...
unsigned char Si2146_L1_DTV_RESTART               (L1_Si2146_Context *api)
{
    ...
    SiTRACE("Si2146 DTV_RESTART ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_DTV_RESTART_CMD;
                                                   ...
                                                   unsigned char   intack)
{
    ...
    SiTRACE("Si2146 DTV_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2146_DTV_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
 ...
 /*---------------------------------------------------*/
/* Si2146_EXIT_BOOTLOADER COMMAND                  */
/*---------------------------------------------------*/
                                                   ...
                                                   unsigned char   ctsien)
{
    ...
    SiTRACE("Si2146 EXIT_BOOTLOADER ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((func   > Si2146_EXIT_BOOTLOADER_CMD_FUNC_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "  , func   );
    if ((ctsien > Si2146_EXIT_BOOTLOADER_CMD_CTSIEN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CTSIEN %d ", ctsien );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
                                                             ...
                                                             int   offset_500hz)
{
    ...
    SiTRACE("Si2146 FINE_TUNE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((reserved     > Si2146_FINE_TUNE_CMD_RESERVED_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED %d "    , reserved     );
    if ((offset_500hz > Si2146_FINE_TUNE_CMD_OFFSET_500HZ_MAX)  || (offset_500hz < Si2146_FINE_TUNE_CMD_OFFSET_500HZ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("OFFSET_500HZ %d ", offset_500hz );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_FINE_TUNE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( reserved     & Si2146_FINE_TUNE_CMD_RESERVED_MASK     ) << Si2146_FINE_TUNE_CMD_RESERVED_LSB    );
    cmdByteBuffer[2] = (unsigned char) ( ( offset_500hz & Si2146_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2146_FINE_TUNE_CMD_OFFSET_500HZ_LSB);
    cmdByteBuffer[3] = (unsigned char) ((( offset_500hz & Si2146_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2146_FINE_TUNE_CMD_OFFSET_500HZ_LSB)>>8);
                                                   ...
                                                   unsigned int    prop)
{
    ...
    SiTRACE("Si2146 GET_PROPERTY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((reserved > Si2146_GET_PROPERTY_CMD_RESERVED_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED %d ", reserved );
    SiTRACE("PROP %d "    , prop     );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_GET_PROPERTY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( reserved & Si2146_GET_PROPERTY_CMD_RESERVED_MASK ) << Si2146_GET_PROPERTY_CMD_RESERVED_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( prop     & Si2146_GET_PROPERTY_CMD_PROP_MASK     ) << Si2146_GET_PROPERTY_CMD_PROP_LSB    );
    cmdByteBuffer[3] = (unsigned char) ((( prop     & Si2146_GET_PROPERTY_CMD_PROP_MASK     ) << Si2146_GET_PROPERTY_CMD_PROP_LSB    )>>8);
    ...
    api->rsp->get_property.reserved =   (( ( (rspByteBuffer[1]  )) >> Si2146_GET_PROPERTY_RESPONSE_RESERVED_LSB ) & Si2146_GET_PROPERTY_RESPONSE_RESERVED_MASK );
    api->rsp->get_property.data     =   (( ( (rspByteBuffer[2]  ) | (rspByteBuffer[3]  << 8 )) >> Si2146_GET_PROPERTY_RESPONSE_DATA_LSB     ) & Si2146_GET_PROPERTY_RESPONSE_DATA_MASK     );
...
unsigned char Si2146_L1_GET_REV                   (L1_Si2146_Context *api)
{
    ...
    SiTRACE("Si2146 GET_REV ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_GET_REV_CMD;
    ...
    error_code = Si2146_pollForResponse(api, 10, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling GET_REV response\n");
      return error_code;
    }

    api->rsp->get_rev.pn       =   (( ( (rspByteBuffer[1]  )) >> Si2146_GET_REV_RESPONSE_PN_LSB       ) & Si2146_GET_REV_RESPONSE_PN_MASK       );
    api->rsp->get_rev.fwmajor  =   (( ( (rspByteBuffer[2]  )) >> Si2146_GET_REV_RESPONSE_FWMAJOR_LSB  ) & Si2146_GET_REV_RESPONSE_FWMAJOR_MASK  );
    api->rsp->get_rev.fwminor  =   (( ( (rspByteBuffer[3]  )) >> Si2146_GET_REV_RESPONSE_FWMINOR_LSB  ) & Si2146_GET_REV_RESPONSE_FWMINOR_MASK  );
    api->rsp->get_rev.patch    =   (( ( (rspByteBuffer[4]  ) | (rspByteBuffer[5]  << 8 )) >> Si2146_GET_REV_RESPONSE_PATCH_LSB    ) & Si2146_GET_REV_RESPONSE_PATCH_MASK    );
    api->rsp->get_rev.cmpmajor =   (( ( (rspByteBuffer[6]  )) >> Si2146_GET_REV_RESPONSE_CMPMAJOR_LSB ) & Si2146_GET_REV_RESPONSE_CMPMAJOR_MASK );
    api->rsp->get_rev.cmpminor =   (( ( (rspByteBuffer[7]  )) >> Si2146_GET_REV_RESPONSE_CMPMINOR_LSB ) & Si2146_GET_REV_RESPONSE_CMPMINOR_MASK );
    api->rsp->get_rev.cmpbuild =   (( ( (rspByteBuffer[8]  )) >> Si2146_GET_REV_RESPONSE_CMPBUILD_LSB ) & Si2146_GET_REV_RESPONSE_CMPBUILD_MASK );
    api->rsp->get_rev.chiprev  =   (( ( (rspByteBuffer[9]  )) >> Si2146_GET_REV_RESPONSE_CHIPREV_LSB  ) & Si2146_GET_REV_RESPONSE_CHIPREV_MASK  );
...
unsigned char Si2146_L1_POWER_DOWN                (L1_Si2146_Context *api)
{
    ...
    SiTRACE("Si2146 POWER_DOWN ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2146_POWER_DOWN_CMD;
                                                   ...
                                                   unsigned char   wake_up)
{
    ...
    SiTRACE("Si2146 POWER_UP ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode    > Si2146_POWER_UP_CMD_SUBCODE_MAX   )  || (subcode    < Si2146_POWER_UP_CMD_SUBCODE_MIN   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "   , subcode    );
    if ((reserved1  > Si2146_POWER_UP_CMD_RESERVED1_MAX )  || (reserved1  < Si2146_POWER_UP_CMD_RESERVED1_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED1 %d " , reserved1  );
    if ((reserved2  > Si2146_POWER_UP_CMD_RESERVED2_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d " , reserved2  );
    if ((reserved3  > Si2146_POWER_UP_CMD_RESERVED3_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d " , reserved3  );
    if ((clock_mode > Si2146_POWER_UP_CMD_CLOCK_MODE_MAX)  || (clock_mode < Si2146_POWER_UP_CMD_CLOCK_MODE_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d ", clock_mode );
    if ((clock_freq > Si2146_POWER_UP_CMD_CLOCK_FREQ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_FREQ %d ", clock_freq );
    if ((addr_mode  > Si2146_POWER_UP_CMD_ADDR_MODE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("ADDR_MODE %d " , addr_mode  );
    if ((func       > Si2146_POWER_UP_CMD_FUNC_MAX      ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "      , func       );
    if ((ctsien     > Si2146_POWER_UP_CMD_CTSIEN_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CTSIEN %d "    , ctsien     );
    if ((wake_up    > Si2146_POWER_UP_CMD_WAKE_UP_MAX   )  || (wake_up    < Si2146_POWER_UP_CMD_WAKE_UP_MIN   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("WAKE_UP %d "   , wake_up    );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
                                                   ...
                                                   unsigned char   intack)
{
    ...
    SiTRACE("Si2146 TUNER_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2146_TUNER_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
 ...
 /*---------------------------------------------------*/
/* Si2146_TUNER_TUNE_FREQ COMMAND                  */
/*---------------------------------------------------*/
                                                   ...
                                                   unsigned long   freq)
{
    ...
    SiTRACE("Si2146 TUNER_TUNE_FREQ ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((mode > Si2146_TUNER_TUNE_FREQ_CMD_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("MODE %d ", mode );
    if ((freq > Si2146_TUNER_TUNE_FREQ_CMD_FREQ_MAX)  || (freq < Si2146_TUNER_TUNE_FREQ_CMD_FREQ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FREQ %d ", freq );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2146_PARAMETER_OUT_OF_RANGE;
    }
...
unsigned char   Si2146_L1_SendCommand2(L1_Si2146_Context *api, unsigned int cmd_code) {
    switch (cmd_code) {
    #ifdef        Si2146_AGC_OVERRIDE_CMD
     case         Si2146_AGC_OVERRIDE_CMD_CODE:
       return Si2146_L1_AGC_OVERRIDE (api, api->cmd->agc_override.force_max_gain, api->cmd->agc_override.force_top_gain );
     break;
    #endif /*     Si2146_AGC_OVERRIDE_CMD */
    #ifdef        Si2146_ATV_RESTART_CMD
     case         Si2146_ATV_RESTART_CMD_CODE:
       return Si2146_L1_ATV_RESTART (api, api->cmd->atv_restart.mode );
     break;
    #endif /*     Si2146_ATV_RESTART_CMD */
    #ifdef        Si2146_ATV_STATUS_CMD
     case         Si2146_ATV_STATUS_CMD_CODE:
       return Si2146_L1_ATV_STATUS (api, api->cmd->atv_status.intack );
     break;
    #endif /*     Si2146_ATV_STATUS_CMD */
    #ifdef        Si2146_CONFIG_PINS_CMD
     case         Si2146_CONFIG_PINS_CMD_CODE:
       return Si2146_L1_CONFIG_PINS (api, api->cmd->config_pins.gpio1_mode, api->cmd->config_pins.gpio1_read, api->cmd->config_pins.gpio2_mode, api->cmd->config_pins.gpio2_read, api->cmd->config_pins.gpio3_mode, api->cmd->config_pins.gpio3_read, api->cmd->config_pins.bclk1_mode, api->cmd->config_pins.bclk1_read, api->cmd->config_pins.xout_mode );
     break;
    #endif /*     Si2146_CONFIG_PINS_CMD */
    #ifdef        Si2146_DTV_RESTART_CMD
     case         Si2146_DTV_RESTART_CMD_CODE:
       return Si2146_L1_DTV_RESTART (api );
     break;
    #endif /*     Si2146_DTV_RESTART_CMD */
    #ifdef        Si2146_DTV_STATUS_CMD
     case         Si2146_DTV_STATUS_CMD_CODE:
       return Si2146_L1_DTV_STATUS (api, api->cmd->dtv_status.intack );
     break;
    #endif /*     Si2146_DTV_STATUS_CMD */
    #ifdef        Si2146_EXIT_BOOTLOADER_CMD
     case         Si2146_EXIT_BOOTLOADER_CMD_CODE:
       return Si2146_L1_EXIT_BOOTLOADER (api, api->cmd->exit_bootloader.func, api->cmd->exit_bootloader.ctsien );
     break;
    #endif /*     Si2146_EXIT_BOOTLOADER_CMD */
    #ifdef        Si2146_FINE_TUNE_CMD
     case         Si2146_FINE_TUNE_CMD_CODE:
       return Si2146_L1_FINE_TUNE (api, api->cmd->fine_tune.reserved, api->cmd->fine_tune.offset_500hz );
     break;
    #endif /*     Si2146_FINE_TUNE_CMD */
    #ifdef        Si2146_GET_PROPERTY_CMD
     case         Si2146_GET_PROPERTY_CMD_CODE:
       return Si2146_L1_GET_PROPERTY (api, api->cmd->get_property.reserved, api->cmd->get_property.prop );
     break;
    #endif /*     Si2146_GET_PROPERTY_CMD */
    #ifdef        Si2146_GET_REV_CMD
     case         Si2146_GET_REV_CMD_CODE:
       return Si2146_L1_GET_REV (api );
     break;
    #endif /*     Si2146_GET_REV_CMD */
    #ifdef        Si2146_PART_INFO_CMD
     case         Si2146_PART_INFO_CMD_CODE:
       return Si2146_L1_PART_INFO (api );
     break;
    #endif /*     Si2146_PART_INFO_CMD */
    #ifdef        Si2146_POWER_DOWN_CMD
     case         Si2146_POWER_DOWN_CMD_CODE:
       return Si2146_L1_POWER_DOWN (api );
     break;
    #endif /*     Si2146_POWER_DOWN_CMD */
    #ifdef        Si2146_POWER_UP_CMD
     case         Si2146_POWER_UP_CMD_CODE:
       return Si2146_L1_POWER_UP (api, api->cmd->power_up.subcode, api->cmd->power_up.reserved1, api->cmd->power_up.reserved2, api->cmd->power_up.reserved3, api->cmd->power_up.clock_mode, api->cmd->power_up.clock_freq, api->cmd->power_up.addr_mode, api->cmd->power_up.func, api->cmd->power_up.ctsien, api->cmd->power_up.wake_up );
     break;
    #endif /*     Si2146_POWER_UP_CMD */
    #ifdef        Si2146_SET_PROPERTY_CMD
     case         Si2146_SET_PROPERTY_CMD_CODE:
       return Si2146_L1_SET_PROPERTY (api, api->cmd->set_property.reserved, api->cmd->set_property.prop, api->cmd->set_property.data );
     break;
    #endif /*     Si2146_SET_PROPERTY_CMD */
    #ifdef        Si2146_STANDBY_CMD
     case         Si2146_STANDBY_CMD_CODE:
       return Si2146_L1_STANDBY (api );
     break;
    #endif /*     Si2146_STANDBY_CMD */
    #ifdef        Si2146_TUNER_STATUS_CMD
     case         Si2146_TUNER_STATUS_CMD_CODE:
       return Si2146_L1_TUNER_STATUS (api, api->cmd->tuner_status.intack );
     break;
    #endif /*     Si2146_TUNER_STATUS_CMD */
    #ifdef        Si2146_TUNER_TUNE_FREQ_CMD
     case         Si2146_TUNER_TUNE_FREQ_CMD_CODE:
       return Si2146_L1_TUNER_TUNE_FREQ (api, api->cmd->tuner_tune_freq.mode, api->cmd->tuner_tune_freq.freq );
     break;
    #endif /*     Si2146_TUNER_TUNE_FREQ_CMD */
   default : break;
    }
