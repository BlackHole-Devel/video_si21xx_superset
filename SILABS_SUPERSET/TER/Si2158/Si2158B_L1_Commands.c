 ...
 ***********************************************************************************************************************/
unsigned char Si2158B_convert_to_byte (const unsigned char* buffer, unsigned char shift, unsigned char mask) {
  unsigned int rspBuffer = *buffer;
  return ((rspBuffer >> shift) & mask);
}
unsigned long Si2158B_convert_to_ulong(const unsigned char* buffer, unsigned char shift,unsigned long mask) {
  return ((( ( (unsigned long)buffer[0]) | ((unsigned long)buffer[1] << 8) | ((unsigned long)buffer[2]<<16) | ((unsigned long)buffer[3]<<24)) >> shift) & mask );
}
unsigned int  Si2158B_convert_to_uint (const unsigned char* buffer, unsigned char shift,unsigned int mask) {
  return (( ( (unsigned int)buffer[0]) | (((unsigned int)buffer[1]) << 8) >> shift) & mask);
}
short         Si2158B_convert_to_int  (const unsigned char* buffer, unsigned char shift,unsigned int mask) {
  return (( ( (unsigned int)buffer[0]) | (((unsigned int)buffer[1]) << 8) >> shift) & mask);
}
/***********************************************************************************************************************
  Si2158B_CurrentResponseStatus function
  Use:        status checking function
              Used to fill the Si2158B_COMMON_REPLY_struct members with the ptDataBuffer byte's bits
 ...
 ***********************************************************************************************************************/
unsigned char Si2158B_CurrentResponseStatus (L1_Si2158B_Context *api, unsigned char ptDataBuffer)
{
    api->status->tunint =     Si2158B_convert_to_byte(&ptDataBuffer, 0, 0x01);
    api->status->atvint =     Si2158B_convert_to_byte(&ptDataBuffer, 1, 0x01);
    api->status->dtvint =     Si2158B_convert_to_byte(&ptDataBuffer, 2, 0x01);
    api->status->err    =     Si2158B_convert_to_byte(&ptDataBuffer, 6, 0x01);
    api->status->cts    =     Si2158B_convert_to_byte(&ptDataBuffer, 7, 0x01);
  return (api->status->err ? ERROR_Si2158B_ERR : NO_Si2158B_ERROR);
}

 /***********************************************************************************************************************
  Si2158B_pollForResetState function
  Use:        ResetState checking function
              Used to check the status byte for the reset code (0xFE).
  Comments:   Only required for the Si2158B_L1_POWER_DOWN_HW Command

  Returns:   0 if the Status byte is = 0xFE, nonzero otherwise
 ***********************************************************************************************************************/
unsigned char Si2158B_pollForResetState (L1_Si2158B_Context *api)
{
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2158B_pollForResetState ERROR reading byte 0!\n");
      return ERROR_Si2158B_POLLING_CTS;
    }
  ...
  SiTRACE("Si2158B_pollForResetState ERROR status Timeout!\n");
  return ERROR_Si2158B_CTS_TIMEOUT;
}
/***********************************************************************************************************************
  Si2158B_pollForCTS function
 ...
 ***********************************************************************************************************************/
unsigned char Si2158B_pollForCTS (L1_Si2158B_Context *api)
{
  unsigned char rspByteBuffer[1];
  unsigned int start_time;
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      SiTRACE("Si2158B_pollForCTS ERROR reading byte 0!\n");
      return ERROR_Si2158B_POLLING_CTS;
    }
  ...
  SiTRACE("Si2158B_pollForCTS ERROR CTS Timeout!\n");
  return ERROR_Si2158B_CTS_TIMEOUT;
}

/***********************************************************************************************************************
  Si2158B_pollForResponse function
 ...
 ***********************************************************************************************************************/
unsigned char Si2158B_pollForResponse (L1_Si2158B_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  unsigned int start_time;
  ...
  while (system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if ((unsigned int)L0_ReadCommandBytes(api->i2c, nbBytes, pByteBuffer) != nbBytes) {
      SiTRACE("Si2158B_pollForResponse ERROR reading byte 0!\n");
      return ERROR_Si2158B_POLLING_RESPONSE;
    }
    /* return response err flag if CTS set */
    if (pByteBuffer[0] & 0x80)  {
      return Si2158B_CurrentResponseStatus(api, pByteBuffer[0]);
    }
  }

  SiTRACE("Si2158B_pollForResponse ERROR CTS Timeout!\n");
  return ERROR_Si2158B_CTS_TIMEOUT;
}

#ifdef    Si2158B_AGC_OVERRIDE_CMD
/*---------------------------------------------------*/
/* Si2158B_AGC_OVERRIDE COMMAND                         */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_AGC_OVERRIDE    (L1_Si2158B_Context *api,
                                         ...
                                         unsigned char   force_top_gain)
{
    ...
    SiTRACE("Si2158B AGC_OVERRIDE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((force_max_gain > Si2158B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_MAX_GAIN %d ", force_max_gain );
    if ((force_top_gain > Si2158B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FORCE_TOP_GAIN %d ", force_top_gain );
    if (error_code) {
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_AGC_OVERRIDE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( force_max_gain & Si2158B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MASK ) << Si2158B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_LSB|
                                         ( force_top_gain & Si2158B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MASK ) << Si2158B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_LSB);
    ...
    error_code = Si2158B_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
...
/*---------------------------------------------------*/
/* Si2158B_ATV_CW_TEST COMMAND                           */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_ATV_CW_TEST     (L1_Si2158B_Context *api,
                                         unsigned char   pc_lock)
{
    ...
    SiTRACE("Si2158B ATV_CW_TEST ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((pc_lock > Si2158B_ATV_CW_TEST_CMD_PC_LOCK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PC_LOCK %d ", pc_lock );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_ATV_CW_TEST_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( pc_lock & Si2158B_ATV_CW_TEST_CMD_PC_LOCK_MASK ) << Si2158B_ATV_CW_TEST_CMD_PC_LOCK_LSB);
...
/*---------------------------------------------------*/
/* Si2158B_ATV_RESTART COMMAND                           */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_ATV_RESTART     (L1_Si2158B_Context *api)
{
    unsigned char error_code = 0;
    ...
    SiTRACE("Si2158B ATV_RESTART ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_ATV_RESTART_CMD;
...
/*---------------------------------------------------*/
/* Si2158B_ATV_SCAN_TUNE COMMAND                       */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_ATV_SCAN_TUNE   (L1_Si2158B_Context *api,
                                         unsigned long   freq)
{
    unsigned char error_code = 0;
    ...
    SiTRACE("Si2158B ATV_SCAN_TUNE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((freq > Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MAX)  || (freq < Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FREQ %d ", freq );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_ATV_SCAN_TUNE_CMD;
    cmdByteBuffer[1] = (unsigned char)0x00;
    cmdByteBuffer[2] = (unsigned char)0x00;
    cmdByteBuffer[3] = (unsigned char)0x00;
    cmdByteBuffer[4] = (unsigned char) ( ( freq & Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MASK ) << Si2158B_ATV_SCAN_TUNE_CMD_FREQ_LSB);
    cmdByteBuffer[5] = (unsigned char) ((( freq & Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MASK ) << Si2158B_ATV_SCAN_TUNE_CMD_FREQ_LSB)>>8);
    cmdByteBuffer[6] = (unsigned char) ((( freq & Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MASK ) << Si2158B_ATV_SCAN_TUNE_CMD_FREQ_LSB)>>16);
    cmdByteBuffer[7] = (unsigned char) ((( freq & Si2158B_ATV_SCAN_TUNE_CMD_FREQ_MASK ) << Si2158B_ATV_SCAN_TUNE_CMD_FREQ_LSB)>>24);
...
/*---------------------------------------------------*/
/* Si2158B_ATV_STATUS COMMAND                             */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_ATV_STATUS      (L1_Si2158B_Context *api,
                                         unsigned char   intack)
{
    ...
    SiTRACE("Si2158B ATV_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2158B_ATV_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_ATV_STATUS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( intack & Si2158B_ATV_STATUS_CMD_INTACK_MASK ) << Si2158B_ATV_STATUS_CMD_INTACK_LSB);
    ...
    error_code = Si2158B_pollForResponse(api, 9, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling ATV_STATUS response\n");
      return error_code;
    }

    api->rsp->atv_status.chlint    = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_ATV_STATUS_RESPONSE_CHLINT_LSB     , Si2158B_ATV_STATUS_RESPONSE_CHLINT_MASK    );
    api->rsp->atv_status.pclint    = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_ATV_STATUS_RESPONSE_PCLINT_LSB     , Si2158B_ATV_STATUS_RESPONSE_PCLINT_MASK    );
    api->rsp->atv_status.chl       = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_ATV_STATUS_RESPONSE_CHL_LSB        , Si2158B_ATV_STATUS_RESPONSE_CHL_MASK       );
    api->rsp->atv_status.pcl       = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_ATV_STATUS_RESPONSE_PCL_LSB        , Si2158B_ATV_STATUS_RESPONSE_PCL_MASK       );
    api->rsp->atv_status.afc_freq  = Si2158B_convert_to_int   (&rspByteBuffer[ 4] , Si2158B_ATV_STATUS_RESPONSE_AFC_FREQ_LSB   , Si2158B_ATV_STATUS_RESPONSE_AFC_FREQ_MASK  );
    api->rsp->atv_status.video_sys = Si2158B_convert_to_byte  (&rspByteBuffer[ 8] , Si2158B_ATV_STATUS_RESPONSE_VIDEO_SYS_LSB  , Si2158B_ATV_STATUS_RESPONSE_VIDEO_SYS_MASK );
    api->rsp->atv_status.color     = Si2158B_convert_to_byte  (&rspByteBuffer[ 8] , Si2158B_ATV_STATUS_RESPONSE_COLOR_LSB      , Si2158B_ATV_STATUS_RESPONSE_COLOR_MASK     );
...
/*---------------------------------------------------*/
/* Si2158B_CONFIG_CLOCKS COMMAND                       */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_CONFIG_CLOCKS   (L1_Si2158B_Context *api,
                                         ...
                                         unsigned char   en_xout)
{
    ...
    SiTRACE("Si2158B CONFIG_CLOCKS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode    > Si2158B_CONFIG_CLOCKS_CMD_SUBCODE_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "   , subcode    );
    if ((clock_mode > Si2158B_CONFIG_CLOCKS_CMD_CLOCK_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d ", clock_mode );
    if ((en_xout    > Si2158B_CONFIG_CLOCKS_CMD_EN_XOUT_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "   , en_xout    );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_CONFIG_CLOCKS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode    & Si2158B_CONFIG_CLOCKS_CMD_SUBCODE_MASK    ) << Si2158B_CONFIG_CLOCKS_CMD_SUBCODE_LSB   );
    cmdByteBuffer[2] = (unsigned char) ( ( clock_mode & Si2158B_CONFIG_CLOCKS_CMD_CLOCK_MODE_MASK ) << Si2158B_CONFIG_CLOCKS_CMD_CLOCK_MODE_LSB|
                                         ( en_xout    & Si2158B_CONFIG_CLOCKS_CMD_EN_XOUT_MASK    ) << Si2158B_CONFIG_CLOCKS_CMD_EN_XOUT_LSB   );
...
/*---------------------------------------------------*/
/* Si2158B_CONFIG_I2C COMMAND                             */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_CONFIG_I2C      (L1_Si2158B_Context *api,
                                         unsigned char   subcode,
                                         unsigned char   i2c_broadcast)
{
    ...
    SiTRACE("Si2158B CONFIG_I2C ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode       > Si2158B_CONFIG_I2C_CMD_SUBCODE_MAX      )  || (subcode       < Si2158B_CONFIG_I2C_CMD_SUBCODE_MIN      ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "      , subcode       );
    if ((i2c_broadcast > Si2158B_CONFIG_I2C_CMD_I2C_BROADCAST_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("I2C_BROADCAST %d ", i2c_broadcast );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_CONFIG_I2C_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode       & Si2158B_CONFIG_I2C_CMD_SUBCODE_MASK       ) << Si2158B_CONFIG_I2C_CMD_SUBCODE_LSB      );
    cmdByteBuffer[2] = (unsigned char) ( ( i2c_broadcast & Si2158B_CONFIG_I2C_CMD_I2C_BROADCAST_MASK ) << Si2158B_CONFIG_I2C_CMD_I2C_BROADCAST_LSB);
...
/*---------------------------------------------------*/
/* Si2158B_CONFIG_PINS COMMAND                           */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_CONFIG_PINS     (L1_Si2158B_Context *api,
                                         ...
                                         unsigned char   reserved3)
{
    ...
    SiTRACE("Si2158B CONFIG_PINS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((gpio1_mode > Si2158B_CONFIG_PINS_CMD_GPIO1_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_MODE %d ", gpio1_mode );
    if ((gpio1_read > Si2158B_CONFIG_PINS_CMD_GPIO1_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO1_READ %d ", gpio1_read );
    if ((gpio2_mode > Si2158B_CONFIG_PINS_CMD_GPIO2_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_MODE %d ", gpio2_mode );
    if ((gpio2_read > Si2158B_CONFIG_PINS_CMD_GPIO2_READ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("GPIO2_READ %d ", gpio2_read );
    if ((reserved1  > Si2158B_CONFIG_PINS_CMD_RESERVED1_MAX )  || (reserved1  < Si2158B_CONFIG_PINS_CMD_RESERVED1_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED1 %d " , reserved1  );
    if ((reserved2  > Si2158B_CONFIG_PINS_CMD_RESERVED2_MAX )  || (reserved2  < Si2158B_CONFIG_PINS_CMD_RESERVED2_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d " , reserved2  );
    if ((reserved3  > Si2158B_CONFIG_PINS_CMD_RESERVED3_MAX )  || (reserved3  < Si2158B_CONFIG_PINS_CMD_RESERVED3_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d " , reserved3  );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_CONFIG_PINS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( gpio1_mode & Si2158B_CONFIG_PINS_CMD_GPIO1_MODE_MASK ) << Si2158B_CONFIG_PINS_CMD_GPIO1_MODE_LSB|
                                         ( gpio1_read & Si2158B_CONFIG_PINS_CMD_GPIO1_READ_MASK ) << Si2158B_CONFIG_PINS_CMD_GPIO1_READ_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( gpio2_mode & Si2158B_CONFIG_PINS_CMD_GPIO2_MODE_MASK ) << Si2158B_CONFIG_PINS_CMD_GPIO2_MODE_LSB|
                                         ( gpio2_read & Si2158B_CONFIG_PINS_CMD_GPIO2_READ_MASK ) << Si2158B_CONFIG_PINS_CMD_GPIO2_READ_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( reserved1  & Si2158B_CONFIG_PINS_CMD_RESERVED1_MASK  ) << Si2158B_CONFIG_PINS_CMD_RESERVED1_LSB );
    cmdByteBuffer[4] = (unsigned char) ( ( reserved2  & Si2158B_CONFIG_PINS_CMD_RESERVED2_MASK  ) << Si2158B_CONFIG_PINS_CMD_RESERVED2_LSB );
    cmdByteBuffer[5] = (unsigned char) ( ( reserved3  & Si2158B_CONFIG_PINS_CMD_RESERVED3_MASK  ) << Si2158B_CONFIG_PINS_CMD_RESERVED3_LSB );
    ...
    error_code = Si2158B_pollForResponse(api, 6, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling CONFIG_PINS response\n");
      return error_code;
    }

    api->rsp->config_pins.gpio1_mode  = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_CONFIG_PINS_RESPONSE_GPIO1_MODE_LSB   , Si2158B_CONFIG_PINS_RESPONSE_GPIO1_MODE_MASK  );
    api->rsp->config_pins.gpio1_state = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_CONFIG_PINS_RESPONSE_GPIO1_STATE_LSB  , Si2158B_CONFIG_PINS_RESPONSE_GPIO1_STATE_MASK );
    api->rsp->config_pins.gpio2_mode  = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_CONFIG_PINS_RESPONSE_GPIO2_MODE_LSB   , Si2158B_CONFIG_PINS_RESPONSE_GPIO2_MODE_MASK  );
    api->rsp->config_pins.gpio2_state = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_CONFIG_PINS_RESPONSE_GPIO2_STATE_LSB  , Si2158B_CONFIG_PINS_RESPONSE_GPIO2_STATE_MASK );
    api->rsp->config_pins.reserved1   = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_CONFIG_PINS_RESPONSE_RESERVED1_LSB    , Si2158B_CONFIG_PINS_RESPONSE_RESERVED1_MASK   );
    api->rsp->config_pins.reserved2   = Si2158B_convert_to_byte  (&rspByteBuffer[ 4] , Si2158B_CONFIG_PINS_RESPONSE_RESERVED2_LSB    , Si2158B_CONFIG_PINS_RESPONSE_RESERVED2_MASK   );
    api->rsp->config_pins.reserved3   = Si2158B_convert_to_byte  (&rspByteBuffer[ 5] , Si2158B_CONFIG_PINS_RESPONSE_RESERVED3_LSB    , Si2158B_CONFIG_PINS_RESPONSE_RESERVED3_MASK   );
...
/*---------------------------------------------------*/
/* Si2158B_DTV_RESTART COMMAND                           */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_DTV_RESTART     (L1_Si2158B_Context *api)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[1];
    unsigned char rspByteBuffer[1];
    ...
    SiTRACE("Si2158B DTV_RESTART ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_DTV_RESTART_CMD;
...
/*---------------------------------------------------*/
/* Si2158B_DTV_STATUS COMMAND                             */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_DTV_STATUS      (L1_Si2158B_Context *api,
                                         unsigned char   intack)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[2];
    ...
    SiTRACE("Si2158B DTV_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2158B_DTV_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_DTV_STATUS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( intack & Si2158B_DTV_STATUS_CMD_INTACK_MASK ) << Si2158B_DTV_STATUS_CMD_INTACK_LSB);
    ...
    error_code = Si2158B_pollForResponse(api, 4, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling DTV_STATUS response\n");
      return error_code;
    }

    api->rsp->dtv_status.chlint     = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_DTV_STATUS_RESPONSE_CHLINT_LSB      , Si2158B_DTV_STATUS_RESPONSE_CHLINT_MASK     );
    api->rsp->dtv_status.chl        = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_DTV_STATUS_RESPONSE_CHL_LSB         , Si2158B_DTV_STATUS_RESPONSE_CHL_MASK        );
    api->rsp->dtv_status.bw         = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_DTV_STATUS_RESPONSE_BW_LSB          , Si2158B_DTV_STATUS_RESPONSE_BW_MASK         );
    api->rsp->dtv_status.modulation = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_DTV_STATUS_RESPONSE_MODULATION_LSB  , Si2158B_DTV_STATUS_RESPONSE_MODULATION_MASK );
...
/*---------------------------------------------------*/
/* Si2158B_EXIT_BOOTLOADER COMMAND                   */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_EXIT_BOOTLOADER (L1_Si2158B_Context *api,
                                         unsigned char   func,
                                         unsigned char   ctsien)
{
    ...
    SiTRACE("Si2158B EXIT_BOOTLOADER ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((func   > Si2158B_EXIT_BOOTLOADER_CMD_FUNC_MAX  )  || (func   < Si2158B_EXIT_BOOTLOADER_CMD_FUNC_MIN  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "  , func   );
    if ((ctsien > Si2158B_EXIT_BOOTLOADER_CMD_CTSIEN_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CTSIEN %d ", ctsien );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_EXIT_BOOTLOADER_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( func   & Si2158B_EXIT_BOOTLOADER_CMD_FUNC_MASK   ) << Si2158B_EXIT_BOOTLOADER_CMD_FUNC_LSB  |
                                         ( ctsien & Si2158B_EXIT_BOOTLOADER_CMD_CTSIEN_MASK ) << Si2158B_EXIT_BOOTLOADER_CMD_CTSIEN_LSB);
...
/*---------------------------------------------------*/
/* Si2158B_FINE_TUNE COMMAND                               */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_FINE_TUNE       (L1_Si2158B_Context *api,
                                                   ...
                                                   int   offset_500hz)
{
    ...
    SiTRACE("Si2158B FINE_TUNE ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((persistence  > Si2158B_FINE_TUNE_CMD_PERSISTENCE_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PERSISTENCE %d " , persistence  );
    if ((apply_to_lif > Si2158B_FINE_TUNE_CMD_APPLY_TO_LIF_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("APPLY_TO_LIF %d ", apply_to_lif );
    if ((offset_500hz > Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_MAX)  || (offset_500hz < Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("OFFSET_500HZ %d ", offset_500hz );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_FINE_TUNE_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( persistence  & Si2158B_FINE_TUNE_CMD_PERSISTENCE_MASK  ) << Si2158B_FINE_TUNE_CMD_PERSISTENCE_LSB |
                                         ( apply_to_lif & Si2158B_FINE_TUNE_CMD_APPLY_TO_LIF_MASK ) << Si2158B_FINE_TUNE_CMD_APPLY_TO_LIF_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( offset_500hz & Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_LSB);
    cmdByteBuffer[3] = (unsigned char) ((( offset_500hz & Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_MASK ) << Si2158B_FINE_TUNE_CMD_OFFSET_500HZ_LSB)>>8);
...
/*---------------------------------------------------*/
/* Si2158B_GET_PROPERTY COMMAND                         */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_GET_PROPERTY    (L1_Si2158B_Context *api,
                                         ...
                                         unsigned int    prop)
{
    ...
    SiTRACE("Si2158B GET_PROPERTY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((reserved > Si2158B_GET_PROPERTY_CMD_RESERVED_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED %d ", reserved );
    SiTRACE("PROP %d "    , prop     );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_GET_PROPERTY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( reserved & Si2158B_GET_PROPERTY_CMD_RESERVED_MASK ) << Si2158B_GET_PROPERTY_CMD_RESERVED_LSB);
    cmdByteBuffer[2] = (unsigned char) ( ( prop     & Si2158B_GET_PROPERTY_CMD_PROP_MASK     ) << Si2158B_GET_PROPERTY_CMD_PROP_LSB    );
    cmdByteBuffer[3] = (unsigned char) ((( prop     & Si2158B_GET_PROPERTY_CMD_PROP_MASK     ) << Si2158B_GET_PROPERTY_CMD_PROP_LSB    )>>8);
    ...
    api->rsp->get_property.reserved = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_GET_PROPERTY_RESPONSE_RESERVED_LSB  , Si2158B_GET_PROPERTY_RESPONSE_RESERVED_MASK );
    api->rsp->get_property.data     = Si2158B_convert_to_uint  (&rspByteBuffer[ 2] , Si2158B_GET_PROPERTY_RESPONSE_DATA_LSB      , Si2158B_GET_PROPERTY_RESPONSE_DATA_MASK     );
...
/*---------------------------------------------------*/
/* Si2158B_GET_REV COMMAND                                   */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_GET_REV         (L1_Si2158B_Context *api)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[1];
    ...
    SiTRACE("Si2158B GET_REV ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_GET_REV_CMD;
    ...
    error_code = Si2158B_pollForResponse(api, 10, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling GET_REV response\n");
      return error_code;
    }

    api->rsp->get_rev.pn       = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_GET_REV_RESPONSE_PN_LSB        , Si2158B_GET_REV_RESPONSE_PN_MASK       );
    api->rsp->get_rev.fwmajor  = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_GET_REV_RESPONSE_FWMAJOR_LSB   , Si2158B_GET_REV_RESPONSE_FWMAJOR_MASK  );
    api->rsp->get_rev.fwminor  = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_GET_REV_RESPONSE_FWMINOR_LSB   , Si2158B_GET_REV_RESPONSE_FWMINOR_MASK  );
    api->rsp->get_rev.patch    = Si2158B_convert_to_uint  (&rspByteBuffer[ 4] , Si2158B_GET_REV_RESPONSE_PATCH_LSB     , Si2158B_GET_REV_RESPONSE_PATCH_MASK    );
    api->rsp->get_rev.cmpmajor = Si2158B_convert_to_byte  (&rspByteBuffer[ 6] , Si2158B_GET_REV_RESPONSE_CMPMAJOR_LSB  , Si2158B_GET_REV_RESPONSE_CMPMAJOR_MASK );
    api->rsp->get_rev.cmpminor = Si2158B_convert_to_byte  (&rspByteBuffer[ 7] , Si2158B_GET_REV_RESPONSE_CMPMINOR_LSB  , Si2158B_GET_REV_RESPONSE_CMPMINOR_MASK );
    api->rsp->get_rev.cmpbuild = Si2158B_convert_to_byte  (&rspByteBuffer[ 8] , Si2158B_GET_REV_RESPONSE_CMPBUILD_LSB  , Si2158B_GET_REV_RESPONSE_CMPBUILD_MASK );
    api->rsp->get_rev.chiprev  = Si2158B_convert_to_byte  (&rspByteBuffer[ 9] , Si2158B_GET_REV_RESPONSE_CHIPREV_LSB   , Si2158B_GET_REV_RESPONSE_CHIPREV_MASK  );
...
/*---------------------------------------------------*/
/* Si2158B_PART_INFO COMMAND                               */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_PART_INFO       (L1_Si2158B_Context *api)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[1];
    ...
    SiTRACE("Si2158B PART_INFO ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_PART_INFO_CMD;
    ...
    error_code = Si2158B_pollForResponse(api, 13, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling PART_INFO response\n");
      return error_code;
    }

    api->rsp->part_info.chiprev  = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_PART_INFO_RESPONSE_CHIPREV_LSB   , Si2158B_PART_INFO_RESPONSE_CHIPREV_MASK  );
    api->rsp->part_info.part     = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_PART_INFO_RESPONSE_PART_LSB      , Si2158B_PART_INFO_RESPONSE_PART_MASK     );
    api->rsp->part_info.pmajor   = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_PART_INFO_RESPONSE_PMAJOR_LSB    , Si2158B_PART_INFO_RESPONSE_PMAJOR_MASK   );
    api->rsp->part_info.pminor   = Si2158B_convert_to_byte  (&rspByteBuffer[ 4] , Si2158B_PART_INFO_RESPONSE_PMINOR_LSB    , Si2158B_PART_INFO_RESPONSE_PMINOR_MASK   );
    api->rsp->part_info.pbuild   = Si2158B_convert_to_byte  (&rspByteBuffer[ 5] , Si2158B_PART_INFO_RESPONSE_PBUILD_LSB    , Si2158B_PART_INFO_RESPONSE_PBUILD_MASK   );
    api->rsp->part_info.reserved = Si2158B_convert_to_uint  (&rspByteBuffer[ 6] , Si2158B_PART_INFO_RESPONSE_RESERVED_LSB  , Si2158B_PART_INFO_RESPONSE_RESERVED_MASK );
    api->rsp->part_info.serial   = Si2158B_convert_to_ulong (&rspByteBuffer[ 8] , Si2158B_PART_INFO_RESPONSE_SERIAL_LSB    , Si2158B_PART_INFO_RESPONSE_SERIAL_MASK   );
    api->rsp->part_info.romid    = Si2158B_convert_to_byte  (&rspByteBuffer[12] , Si2158B_PART_INFO_RESPONSE_ROMID_LSB     , Si2158B_PART_INFO_RESPONSE_ROMID_MASK    );
...
/*---------------------------------------------------*/
/* Si2158B_POWER_DOWN COMMAND                             */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_POWER_DOWN      (L1_Si2158B_Context *api)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[1];
    unsigned char rspByteBuffer[1];
    ...
    SiTRACE("Si2158B POWER_DOWN ");
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_POWER_DOWN_CMD;
...
/*---------------------------------------------------*/
/* Si2158B_POWER_DOWN_HW COMMAND                       */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_POWER_DOWN_HW   (L1_Si2158B_Context *api,
                                         unsigned char   subcode,
                                         ...
                                         unsigned char   reserved8)
{
    ...
    SiTRACE("Si2158B POWER_DOWN_HW ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2158B_POWER_DOWN_HW_CMD_SUBCODE_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "  , subcode   );
    if ((pd_xo_osc > Si2158B_POWER_DOWN_HW_CMD_PD_XO_OSC_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PD_XO_OSC %d ", pd_xo_osc );
    if ((reserved1 > Si2158B_POWER_DOWN_HW_CMD_RESERVED1_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED1 %d ", reserved1 );
    if ((en_xout   > Si2158B_POWER_DOWN_HW_CMD_EN_XOUT_MAX  ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "  , en_xout   );
    if ((reserved2 > Si2158B_POWER_DOWN_HW_CMD_RESERVED2_MAX)  || (reserved2 < Si2158B_POWER_DOWN_HW_CMD_RESERVED2_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d ", reserved2 );
    if ((pd_ldo    > Si2158B_POWER_DOWN_HW_CMD_PD_LDO_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PD_LDO %d "   , pd_ldo    );
    if ((reserved3 > Si2158B_POWER_DOWN_HW_CMD_RESERVED3_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d ", reserved3 );
    if ((reserved4 > Si2158B_POWER_DOWN_HW_CMD_RESERVED4_MAX)  || (reserved4 < Si2158B_POWER_DOWN_HW_CMD_RESERVED4_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED4 %d ", reserved4 );
    if ((reserved5 > Si2158B_POWER_DOWN_HW_CMD_RESERVED5_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED5 %d ", reserved5 );
    if ((reserved6 > Si2158B_POWER_DOWN_HW_CMD_RESERVED6_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED6 %d ", reserved6 );
    if ((reserved7 > Si2158B_POWER_DOWN_HW_CMD_RESERVED7_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED7 %d ", reserved7 );
    if ((reserved8 > Si2158B_POWER_DOWN_HW_CMD_RESERVED8_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED8 %d ", reserved8 );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_POWER_DOWN_HW_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode   & Si2158B_POWER_DOWN_HW_CMD_SUBCODE_MASK   ) << Si2158B_POWER_DOWN_HW_CMD_SUBCODE_LSB  );
    cmdByteBuffer[2] = (unsigned char) ( ( pd_xo_osc & Si2158B_POWER_DOWN_HW_CMD_PD_XO_OSC_MASK ) << Si2158B_POWER_DOWN_HW_CMD_PD_XO_OSC_LSB|
                                         ( reserved1 & Si2158B_POWER_DOWN_HW_CMD_RESERVED1_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED1_LSB|
                                         ( en_xout   & Si2158B_POWER_DOWN_HW_CMD_EN_XOUT_MASK   ) << Si2158B_POWER_DOWN_HW_CMD_EN_XOUT_LSB  |
                                         ( reserved2 & Si2158B_POWER_DOWN_HW_CMD_RESERVED2_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED2_LSB);
    cmdByteBuffer[3] = (unsigned char) ( ( pd_ldo    & Si2158B_POWER_DOWN_HW_CMD_PD_LDO_MASK    ) << Si2158B_POWER_DOWN_HW_CMD_PD_LDO_LSB   );
    cmdByteBuffer[4] = (unsigned char) ( ( reserved3 & Si2158B_POWER_DOWN_HW_CMD_RESERVED3_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED3_LSB);
    cmdByteBuffer[5] = (unsigned char) ( ( reserved4 & Si2158B_POWER_DOWN_HW_CMD_RESERVED4_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED4_LSB);
    cmdByteBuffer[6] = (unsigned char) ( ( reserved5 & Si2158B_POWER_DOWN_HW_CMD_RESERVED5_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED5_LSB);
    cmdByteBuffer[7] = (unsigned char) ( ( reserved6 & Si2158B_POWER_DOWN_HW_CMD_RESERVED6_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED6_LSB);
    cmdByteBuffer[8] = (unsigned char) ( ( reserved7 & Si2158B_POWER_DOWN_HW_CMD_RESERVED7_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED7_LSB);
    cmdByteBuffer[9] = (unsigned char) ( ( reserved8 & Si2158B_POWER_DOWN_HW_CMD_RESERVED8_MASK ) << Si2158B_POWER_DOWN_HW_CMD_RESERVED8_LSB);
    ...
    error_code = Si2158B_pollForResetState(api);
    if (error_code) {
...
/*---------------------------------------------------*/
/* Si2158B_POWER_UP COMMAND                                 */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_POWER_UP        (L1_Si2158B_Context *api,
                                         unsigned char   subcode,
                                         unsigned char   clock_mode,
                                         unsigned char   en_xout,
                                         unsigned char   pd_ldo,
                                         ...
                                         unsigned char   wake_up)
{
    ...
    SiTRACE("Si2158B POWER_UP ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode    > Si2158B_POWER_UP_CMD_SUBCODE_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("SUBCODE %d "   , subcode    );
    if ((clock_mode > Si2158B_POWER_UP_CMD_CLOCK_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_MODE %d ", clock_mode );
    if ((en_xout    > Si2158B_POWER_UP_CMD_EN_XOUT_MAX   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("EN_XOUT %d "   , en_xout    );
    if ((pd_ldo     > Si2158B_POWER_UP_CMD_PD_LDO_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("PD_LDO %d "    , pd_ldo     );
    if ((reserved2  > Si2158B_POWER_UP_CMD_RESERVED2_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED2 %d " , reserved2  );
    if ((reserved3  > Si2158B_POWER_UP_CMD_RESERVED3_MAX )  || (reserved3  < Si2158B_POWER_UP_CMD_RESERVED3_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED3 %d " , reserved3  );
    if ((reserved4  > Si2158B_POWER_UP_CMD_RESERVED4_MAX )  || (reserved4  < Si2158B_POWER_UP_CMD_RESERVED4_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED4 %d " , reserved4  );
    if ((reserved5  > Si2158B_POWER_UP_CMD_RESERVED5_MAX )  || (reserved5  < Si2158B_POWER_UP_CMD_RESERVED5_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED5 %d " , reserved5  );
    if ((reserved6  > Si2158B_POWER_UP_CMD_RESERVED6_MAX )  || (reserved6  < Si2158B_POWER_UP_CMD_RESERVED6_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED6 %d " , reserved6  );
    if ((reserved7  > Si2158B_POWER_UP_CMD_RESERVED7_MAX )  || (reserved7  < Si2158B_POWER_UP_CMD_RESERVED7_MIN ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED7 %d " , reserved7  );
    if ((reset      > Si2158B_POWER_UP_CMD_RESET_MAX     )  || (reset      < Si2158B_POWER_UP_CMD_RESET_MIN     ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESET %d "     , reset      );
    if ((clock_freq > Si2158B_POWER_UP_CMD_CLOCK_FREQ_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CLOCK_FREQ %d ", clock_freq );
    if ((reserved8  > Si2158B_POWER_UP_CMD_RESERVED8_MAX ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("RESERVED8 %d " , reserved8  );
    if ((func       > Si2158B_POWER_UP_CMD_FUNC_MAX      ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FUNC %d "      , func       );
    if ((ctsien     > Si2158B_POWER_UP_CMD_CTSIEN_MAX    ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("CTSIEN %d "    , ctsien     );
    if ((wake_up    > Si2158B_POWER_UP_CMD_WAKE_UP_MAX   )  || (wake_up    < Si2158B_POWER_UP_CMD_WAKE_UP_MIN   ) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("WAKE_UP %d "   , wake_up    );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_POWER_UP_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( subcode    & Si2158B_POWER_UP_CMD_SUBCODE_MASK    ) << Si2158B_POWER_UP_CMD_SUBCODE_LSB   );
    cmdByteBuffer[2] = (unsigned char) ( ( clock_mode & Si2158B_POWER_UP_CMD_CLOCK_MODE_MASK ) << Si2158B_POWER_UP_CMD_CLOCK_MODE_LSB|
                                         ( en_xout    & Si2158B_POWER_UP_CMD_EN_XOUT_MASK    ) << Si2158B_POWER_UP_CMD_EN_XOUT_LSB   );
    cmdByteBuffer[3] = (unsigned char) ( ( pd_ldo     & Si2158B_POWER_UP_CMD_PD_LDO_MASK     ) << Si2158B_POWER_UP_CMD_PD_LDO_LSB    );
    cmdByteBuffer[4] = (unsigned char) ( ( reserved2  & Si2158B_POWER_UP_CMD_RESERVED2_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED2_LSB );
    cmdByteBuffer[5] = (unsigned char) ( ( reserved3  & Si2158B_POWER_UP_CMD_RESERVED3_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED3_LSB );
    cmdByteBuffer[6] = (unsigned char) ( ( reserved4  & Si2158B_POWER_UP_CMD_RESERVED4_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED4_LSB );
    cmdByteBuffer[7] = (unsigned char) ( ( reserved5  & Si2158B_POWER_UP_CMD_RESERVED5_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED5_LSB );
    cmdByteBuffer[8] = (unsigned char) ( ( reserved6  & Si2158B_POWER_UP_CMD_RESERVED6_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED6_LSB );
    cmdByteBuffer[9] = (unsigned char) ( ( reserved7  & Si2158B_POWER_UP_CMD_RESERVED7_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED7_LSB );
    cmdByteBuffer[10] = (unsigned char) ( ( reset      & Si2158B_POWER_UP_CMD_RESET_MASK      ) << Si2158B_POWER_UP_CMD_RESET_LSB     );
    cmdByteBuffer[11] = (unsigned char) ( ( clock_freq & Si2158B_POWER_UP_CMD_CLOCK_FREQ_MASK ) << Si2158B_POWER_UP_CMD_CLOCK_FREQ_LSB);
    cmdByteBuffer[12] = (unsigned char) ( ( reserved8  & Si2158B_POWER_UP_CMD_RESERVED8_MASK  ) << Si2158B_POWER_UP_CMD_RESERVED8_LSB );
    cmdByteBuffer[13] = (unsigned char) ( ( func       & Si2158B_POWER_UP_CMD_FUNC_MASK       ) << Si2158B_POWER_UP_CMD_FUNC_LSB      |
                                         ( ctsien     & Si2158B_POWER_UP_CMD_CTSIEN_MASK     ) << Si2158B_POWER_UP_CMD_CTSIEN_LSB    );
    cmdByteBuffer[14] = (unsigned char) ( ( wake_up    & Si2158B_POWER_UP_CMD_WAKE_UP_MASK    ) << Si2158B_POWER_UP_CMD_WAKE_UP_LSB   );
...
/*---------------------------------------------------*/
/* Si2158B_SET_PROPERTY COMMAND                         */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_SET_PROPERTY    (L1_Si2158B_Context *api,
                                         unsigned char   reserved,
                                         unsigned int    prop,
                                         unsigned int    data)
{
   ...
   res = Si2158B_PackProperty(api->propShadow, prop, (int *) &shadowData);

    SiTRACE("Si2158B SET_PROPERTY ");
  #ifdef   DEBUG_RANGE_CHECK
...
/*This function will only download the property if required.
   ...
   if ((data != shadowData)  || (api->propertyWriteMode == DOWNLOAD_ALWAYS) || (res== ERROR_Si2158B_UNKNOWN_PROPERTY) )
    {
        SiTRACE("\n");
        cmdByteBuffer[0] = Si2158B_SET_PROPERTY_CMD;
        cmdByteBuffer[1] = (unsigned char) ( ( reserved & Si2158B_SET_PROPERTY_CMD_RESERVED_MASK ) << Si2158B_SET_PROPERTY_CMD_RESERVED_LSB);
        cmdByteBuffer[2] = (unsigned char) ( ( prop     & Si2158B_SET_PROPERTY_CMD_PROP_MASK     ) << Si2158B_SET_PROPERTY_CMD_PROP_LSB    );
        cmdByteBuffer[3] = (unsigned char) ((( prop     & Si2158B_SET_PROPERTY_CMD_PROP_MASK     ) << Si2158B_SET_PROPERTY_CMD_PROP_LSB    )>>8);
        cmdByteBuffer[4] = (unsigned char) ( ( data     & Si2158B_SET_PROPERTY_CMD_DATA_MASK     ) << Si2158B_SET_PROPERTY_CMD_DATA_LSB    );
        cmdByteBuffer[5] = (unsigned char) ((( data     & Si2158B_SET_PROPERTY_CMD_DATA_MASK     ) << Si2158B_SET_PROPERTY_CMD_DATA_LSB    )>>8);
    ...
    api->rsp->set_property.reserved = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_SET_PROPERTY_RESPONSE_RESERVED_LSB  , Si2158B_SET_PROPERTY_RESPONSE_RESERVED_MASK );
    api->rsp->set_property.data     = Si2158B_convert_to_uint  (&rspByteBuffer[ 2] , Si2158B_SET_PROPERTY_RESPONSE_DATA_LSB      , Si2158B_SET_PROPERTY_RESPONSE_DATA_MASK     );
      /* Update propShadow with latest data if correctly set */
        Si2158B_UnpackProperty(api->propShadow, prop, (int) data);
    }
    else
    {
        SiTRACE("\nSi2158B_L1_SetProperty: Skipping Property Setting 0x%04x because value unchanged 0x%04x\n", prop,data);
...
/*---------------------------------------------------*/
/* Si2158B_STANDBY COMMAND                                   */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_STANDBY         (L1_Si2158B_Context *api,
                                         unsigned char   type)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[2];
    ...
    SiTRACE("Si2158B STANDBY ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((type > Si2158B_STANDBY_CMD_TYPE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("TYPE %d ", type );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_STANDBY_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( type & Si2158B_STANDBY_CMD_TYPE_MASK ) << Si2158B_STANDBY_CMD_TYPE_LSB);
...
/*---------------------------------------------------*/
/* Si2158B_TUNER_STATUS COMMAND                         */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_TUNER_STATUS    (L1_Si2158B_Context *api,
                                         unsigned char   intack)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[2];
    ...
    SiTRACE("Si2158B TUNER_STATUS ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((intack > Si2158B_TUNER_STATUS_CMD_INTACK_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("INTACK %d ", intack );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_TUNER_STATUS_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( intack & Si2158B_TUNER_STATUS_CMD_INTACK_MASK ) << Si2158B_TUNER_STATUS_CMD_INTACK_LSB);
    ...
    error_code = Si2158B_pollForResponse(api, 12, rspByteBuffer);
    if (error_code) {
      SiTRACE("Error polling TUNER_STATUS response\n");
      return error_code;
    }

    api->rsp->tuner_status.tcint    = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_TUNER_STATUS_RESPONSE_TCINT_LSB     , Si2158B_TUNER_STATUS_RESPONSE_TCINT_MASK    );
    api->rsp->tuner_status.rssilint = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_TUNER_STATUS_RESPONSE_RSSILINT_LSB  , Si2158B_TUNER_STATUS_RESPONSE_RSSILINT_MASK );
    api->rsp->tuner_status.rssihint = Si2158B_convert_to_byte  (&rspByteBuffer[ 1] , Si2158B_TUNER_STATUS_RESPONSE_RSSIHINT_LSB  , Si2158B_TUNER_STATUS_RESPONSE_RSSIHINT_MASK );
    api->rsp->tuner_status.tc       = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_TUNER_STATUS_RESPONSE_TC_LSB        , Si2158B_TUNER_STATUS_RESPONSE_TC_MASK       );
    api->rsp->tuner_status.rssil    = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_TUNER_STATUS_RESPONSE_RSSIL_LSB     , Si2158B_TUNER_STATUS_RESPONSE_RSSIL_MASK    );
    api->rsp->tuner_status.rssih    = Si2158B_convert_to_byte  (&rspByteBuffer[ 2] , Si2158B_TUNER_STATUS_RESPONSE_RSSIH_LSB     , Si2158B_TUNER_STATUS_RESPONSE_RSSIH_MASK    );
    api->rsp->tuner_status.rssi     = Si2158B_convert_to_byte  (&rspByteBuffer[ 3] , Si2158B_TUNER_STATUS_RESPONSE_RSSI_LSB      , Si2158B_TUNER_STATUS_RESPONSE_RSSI_MASK     );
    api->rsp->tuner_status.freq     = Si2158B_convert_to_ulong (&rspByteBuffer[ 4] , Si2158B_TUNER_STATUS_RESPONSE_FREQ_LSB      , Si2158B_TUNER_STATUS_RESPONSE_FREQ_MASK     );
    api->rsp->tuner_status.mode     = Si2158B_convert_to_byte  (&rspByteBuffer[ 8] , Si2158B_TUNER_STATUS_RESPONSE_MODE_LSB      , Si2158B_TUNER_STATUS_RESPONSE_MODE_MASK     );
    api->rsp->tuner_status.vco_code = Si2158B_convert_to_int   (&rspByteBuffer[10] , Si2158B_TUNER_STATUS_RESPONSE_VCO_CODE_LSB  , Si2158B_TUNER_STATUS_RESPONSE_VCO_CODE_MASK );
...
/*---------------------------------------------------*/
/* Si2158B_TUNER_TUNE_FREQ COMMAND                   */
/*---------------------------------------------------*/
unsigned char Si2158B_L1_TUNER_TUNE_FREQ (L1_Si2158B_Context *api,
                                         unsigned char   mode,
                                         unsigned long   freq)
{
    unsigned char error_code = 0;
    ...
    SiTRACE("Si2158B TUNER_TUNE_FREQ ");
  #ifdef   DEBUG_RANGE_CHECK
    if ((mode > Si2158B_TUNER_TUNE_FREQ_CMD_MODE_MAX) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("MODE %d ", mode );
    if ((freq > Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MAX)  || (freq < Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MIN) ) {error_code++; SiTRACE("\nOut of range: ");}; SiTRACE("FREQ %d ", freq );
    if (error_code) {
      SiTRACE("%d out of range parameters\n", error_code);
      return ERROR_Si2158B_PARAMETER_OUT_OF_RANGE;
    }
    ...
    SiTRACE("\n");
    cmdByteBuffer[0] = Si2158B_TUNER_TUNE_FREQ_CMD;
    cmdByteBuffer[1] = (unsigned char) ( ( mode & Si2158B_TUNER_TUNE_FREQ_CMD_MODE_MASK ) << Si2158B_TUNER_TUNE_FREQ_CMD_MODE_LSB);
    cmdByteBuffer[2] = (unsigned char)0x00;
    cmdByteBuffer[3] = (unsigned char)0x00;
    cmdByteBuffer[4] = (unsigned char) ( ( freq & Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_LSB);
    cmdByteBuffer[5] = (unsigned char) ((( freq & Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>8);
    cmdByteBuffer[6] = (unsigned char) ((( freq & Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>16);
    cmdByteBuffer[7] = (unsigned char) ((( freq & Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_MASK ) << Si2158B_TUNER_TUNE_FREQ_CMD_FREQ_LSB)>>24);
  ...
  /* --------------------------------------------*/
unsigned char   Si2158B_L1_SendCommand2(L1_Si2158B_Context *api, unsigned int cmd_code) {
    switch (cmd_code) {
    #ifdef        Si2158B_AGC_OVERRIDE_CMD
     case         Si2158B_AGC_OVERRIDE_CMD_CODE:
       return Si2158B_L1_AGC_OVERRIDE (api, api->cmd->agc_override.force_max_gain, api->cmd->agc_override.force_top_gain );
     break;
    #endif /*     Si2158B_AGC_OVERRIDE_CMD */
    #ifdef        Si2158B_ATV_CW_TEST_CMD
     case         Si2158B_ATV_CW_TEST_CMD_CODE:
       return Si2158B_L1_ATV_CW_TEST (api, api->cmd->atv_cw_test.pc_lock );
     break;
    #endif /*     Si2158B_ATV_CW_TEST_CMD */
    #ifdef        Si2158B_ATV_RESTART_CMD
     case         Si2158B_ATV_RESTART_CMD_CODE:
       return Si2158B_L1_ATV_RESTART (api );
     break;
    #endif /*     Si2158B_ATV_RESTART_CMD */
    #ifdef        Si2158B_ATV_SCAN_TUNE_CMD
     case         Si2158B_ATV_SCAN_TUNE_CMD_CODE:
       return Si2158B_L1_ATV_SCAN_TUNE (api, api->cmd->atv_scan_tune.freq );
     break;
    #endif /*     Si2158B_ATV_SCAN_TUNE_CMD */
    #ifdef        Si2158B_ATV_STATUS_CMD
     case         Si2158B_ATV_STATUS_CMD_CODE:
       return Si2158B_L1_ATV_STATUS (api, api->cmd->atv_status.intack );
     break;
    #endif /*     Si2158B_ATV_STATUS_CMD */
    #ifdef        Si2158B_CONFIG_CLOCKS_CMD
     case         Si2158B_CONFIG_CLOCKS_CMD_CODE:
       return Si2158B_L1_CONFIG_CLOCKS (api, api->cmd->config_clocks.subcode, api->cmd->config_clocks.clock_mode, api->cmd->config_clocks.en_xout );
     break;
    #endif /*     Si2158B_CONFIG_CLOCKS_CMD */
    #ifdef        Si2158B_CONFIG_I2C_CMD
     case         Si2158B_CONFIG_I2C_CMD_CODE:
       return Si2158B_L1_CONFIG_I2C (api, api->cmd->config_i2c.subcode, api->cmd->config_i2c.i2c_broadcast );
     break;
    #endif /*     Si2158B_CONFIG_I2C_CMD */
    #ifdef        Si2158B_CONFIG_PINS_CMD
     case         Si2158B_CONFIG_PINS_CMD_CODE:
       return Si2158B_L1_CONFIG_PINS (api, api->cmd->config_pins.gpio1_mode, api->cmd->config_pins.gpio1_read, api->cmd->config_pins.gpio2_mode, api->cmd->config_pins.gpio2_read, api->cmd->config_pins.reserved1, api->cmd->config_pins.reserved2, api->cmd->config_pins.reserved3 );
     break;
    #endif /*     Si2158B_CONFIG_PINS_CMD */
    #ifdef        Si2158B_DTV_RESTART_CMD
     case         Si2158B_DTV_RESTART_CMD_CODE:
       return Si2158B_L1_DTV_RESTART (api );
     break;
    #endif /*     Si2158B_DTV_RESTART_CMD */
    #ifdef        Si2158B_DTV_STATUS_CMD
     case         Si2158B_DTV_STATUS_CMD_CODE:
       return Si2158B_L1_DTV_STATUS (api, api->cmd->dtv_status.intack );
     break;
    #endif /*     Si2158B_DTV_STATUS_CMD */
    #ifdef        Si2158B_EXIT_BOOTLOADER_CMD
     case         Si2158B_EXIT_BOOTLOADER_CMD_CODE:
       return Si2158B_L1_EXIT_BOOTLOADER (api, api->cmd->exit_bootloader.func, api->cmd->exit_bootloader.ctsien );
     break;
    #endif /*     Si2158B_EXIT_BOOTLOADER_CMD */
    #ifdef        Si2158B_FINE_TUNE_CMD
     case         Si2158B_FINE_TUNE_CMD_CODE:
       return Si2158B_L1_FINE_TUNE (api, api->cmd->fine_tune.persistence, api->cmd->fine_tune.apply_to_lif, api->cmd->fine_tune.offset_500hz );
     break;
    #endif /*     Si2158B_FINE_TUNE_CMD */
    #ifdef        Si2158B_GET_PROPERTY_CMD
     case         Si2158B_GET_PROPERTY_CMD_CODE:
       return Si2158B_L1_GET_PROPERTY (api, api->cmd->get_property.reserved, api->cmd->get_property.prop );
     break;
    #endif /*     Si2158B_GET_PROPERTY_CMD */
    #ifdef        Si2158B_GET_REV_CMD
     case         Si2158B_GET_REV_CMD_CODE:
       return Si2158B_L1_GET_REV (api );
     break;
    #endif /*     Si2158B_GET_REV_CMD */
    #ifdef        Si2158B_PART_INFO_CMD
     case         Si2158B_PART_INFO_CMD_CODE:
       return Si2158B_L1_PART_INFO (api );
     break;
    #endif /*     Si2158B_PART_INFO_CMD */
    #ifdef        Si2158B_POWER_DOWN_CMD
     case         Si2158B_POWER_DOWN_CMD_CODE:
       return Si2158B_L1_POWER_DOWN (api );
     break;
    #endif /*     Si2158B_POWER_DOWN_CMD */
    #ifdef        Si2158B_POWER_DOWN_HW_CMD
     case         Si2158B_POWER_DOWN_HW_CMD_CODE:
       return Si2158B_L1_POWER_DOWN_HW (api, api->cmd->power_down_hw.subcode, api->cmd->power_down_hw.pd_xo_osc, api->cmd->power_down_hw.reserved1, api->cmd->power_down_hw.en_xout, api->cmd->power_down_hw.reserved2, api->cmd->power_down_hw.pd_ldo, api->cmd->power_down_hw.reserved3, api->cmd->power_down_hw.reserved4, api->cmd->power_down_hw.reserved5, api->cmd->power_down_hw.reserved6, api->cmd->power_down_hw.reserved7, api->cmd->power_down_hw.reserved8 );
     break;
    #endif /*     Si2158B_POWER_DOWN_HW_CMD */
    #ifdef        Si2158B_POWER_UP_CMD
     case         Si2158B_POWER_UP_CMD_CODE:
       return Si2158B_L1_POWER_UP (api, api->cmd->power_up.subcode, api->cmd->power_up.clock_mode, api->cmd->power_up.en_xout, api->cmd->power_up.pd_ldo, api->cmd->power_up.reserved2, api->cmd->power_up.reserved3, api->cmd->power_up.reserved4, api->cmd->power_up.reserved5, api->cmd->power_up.reserved6, api->cmd->power_up.reserved7, api->cmd->power_up.reset, api->cmd->power_up.clock_freq, api->cmd->power_up.reserved8, api->cmd->power_up.func, api->cmd->power_up.ctsien, api->cmd->power_up.wake_up );
     break;
    #endif /*     Si2158B_POWER_UP_CMD */
    #ifdef        Si2158B_SET_PROPERTY_CMD
     case         Si2158B_SET_PROPERTY_CMD_CODE:
       return Si2158B_L1_SET_PROPERTY (api, api->cmd->set_property.reserved, api->cmd->set_property.prop, api->cmd->set_property.data );
     break;
    #endif /*     Si2158B_SET_PROPERTY_CMD */
    #ifdef        Si2158B_STANDBY_CMD
     case         Si2158B_STANDBY_CMD_CODE:
       return Si2158B_L1_STANDBY (api, api->cmd->standby.type );
     break;
    #endif /*     Si2158B_STANDBY_CMD */
    #ifdef        Si2158B_TUNER_STATUS_CMD
     case         Si2158B_TUNER_STATUS_CMD_CODE:
       return Si2158B_L1_TUNER_STATUS (api, api->cmd->tuner_status.intack );
     break;
    #endif /*     Si2158B_TUNER_STATUS_CMD */
    #ifdef        Si2158B_TUNER_TUNE_FREQ_CMD
     case         Si2158B_TUNER_TUNE_FREQ_CMD_CODE:
       return Si2158B_L1_TUNER_TUNE_FREQ (api, api->cmd->tuner_tune_freq.mode, api->cmd->tuner_tune_freq.freq );
     break;
    #endif /*     Si2158B_TUNER_TUNE_FREQ_CMD */
   default : break;
    }
...
#ifdef    Si2158B_GET_COMMAND_STRINGS

  /* --------------------------------------------*/
  /* GET_COMMAND_RESPONSE_STRING FUNCTION        */
  /* --------------------------------------------*/
unsigned char   Si2158B_L1_GetCommandResponseString(L1_Si2158B_Context *api, unsigned int cmd_code, const char *separator, char *msg) {
    switch (cmd_code) {
    #ifdef        Si2158B_AGC_OVERRIDE_CMD
     case         Si2158B_AGC_OVERRIDE_CMD_CODE:
...
#endif /* Si2158B_GET_COMMAND_STRINGS */
