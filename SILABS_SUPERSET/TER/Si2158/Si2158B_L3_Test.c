 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2158B Layer 3 API


   L3 sample application
   FILE: Si2158B_L3_Test.c
   ...
   Date: June 23 2016
****************************************************************************************/
#include "Si2158B_L3_Test.h"

/* define TUNER_ONLY if using a Si2158B tuner without demod                                                    */
...
/* define Si2158B_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2158B_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2158B_COMMAND_LINE_APPLICATION */
...
L1_Si2158B_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2158B_Context         *Si2158B;
L1_Si2158B_Context         *front_end;
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2158B_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2158B_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2158B_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2158B_UserInputString function
...
************************************************************************************************************************/
int  Si2158B_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2158B_trace                          (L1_Si2158B_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2158B_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2158B EVB
...
/************************************************************************************************************************
  NAME: Si2158B_DisplayStatusByte
  DESCRIPTION:Read Si2158B STATUS byte and display status bits
  Parameter:  Si2158B Context (I2C address)
  Returns:    Si2158B/I2C transaction error code
************************************************************************************************************************/
int  Si2158B_DisplayStatusByte              (L1_Si2158B_Context *api)
{
    int error_code;

    error_code = Si2158B_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2158B_GetRevision
  DESCRIPTION:  Execute Si2158B GET_REV function and display response fields
  Parameter:  Si2158B Context (I2C address)
...
int  Si2158B_GetRevision                    (L1_Si2158B_Context *api)
{
    ...
    if (Si2158B_L1_GET_REV(api) != NO_Si2158B_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2158B_SENDING_COMMAND;
    }
    Si2158B_L1_GetCommandResponseString(api, Si2158B_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2158B_ATVStatus
...
int Si2158B_ATVStatus (L1_Si2158B_Context *Si2158B)
{
    ...
    if (Si2158B_L1_ATV_STATUS(Si2158B, Si2158B_ATV_STATUS_CMD_INTACK_CLEAR) != NO_Si2158B_ERROR) { /* execute ATV_STATUS and always read response */
        printf("Error reading ATV_STATUS\n");
        return ERROR_Si2158B_SENDING_COMMAND;
    }
   Si2158B_L1_GetCommandResponseString(Si2158B, Si2158B_ATV_STATUS_CMD_CODE, "\n", message);
   printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  Si2158B_DTVStatus function
...
int Si2158B_DTVStatus (L1_Si2158B_Context *Si2158B)
{
    ...
    if (Si2158B_L1_DTV_STATUS(Si2158B, Si2158B_DTV_STATUS_CMD_INTACK_CLEAR) != 0) { /* execute DTV_STATUS and always read response */
        printf("Error reading DTV_STATUS\n");
        return ERROR_Si2158B_SENDING_COMMAND;
    }
  Si2158B_L1_GetCommandResponseString(Si2158B, Si2158B_DTV_STATUS_CMD_CODE,"\n", message);                  /* display results */
  printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2158B_TunerStatus
...
int Si2158B_TunerStatus (L1_Si2158B_Context *Si2158B)
{
    ...
    if (Si2158B_L1_TUNER_STATUS(Si2158B, Si2158B_TUNER_STATUS_CMD_INTACK_CLEAR) != NO_Si2158B_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2158B_SENDING_COMMAND;
    }
  Si2158B_L1_GetCommandResponseString(Si2158B, Si2158B_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2158B_GetUserFrequency
...
----------------------------------------------------------------------------\n\
       This is a demonstration application used to illustrate how to use    \n\
            the Si2158B API delivered by Silicon Laboratories                \n\
                                                                            \n\
...
/************************************************************************************************************************
  NAME: Si2158B_menu
  DESCRIPTION: Si2158B user menu function
...
");
  ...
  printf("\
InitAndConfig  : Initialize and Configure the Si2158B\n\
Status         : Read Si2158B STATUS\n\
...
************************************************************************************************************************/
int Si2158B_demoLoop (L1_Si2158B_Context *Si2158B, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2158B_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2158B_trace (Si2158B); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2158B_Init(Si2158B) == 0 ) {
           Si2158B_Configure(Si2158B);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2158B at 0x%02x!\n", Si2158B->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2158B_DisplayStatusByte     (Si2158B); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2158B_GetRevision           (Si2158B); }
#ifdef USING_ALIF_FILTER
    else if (strcmp_nocase(entry, "LoadALIFVidFilt"   )==0) { Si2158B_LoadVideofilter(Si2158B,ALIF_VIDFILT_TABLE,ALIF_VIDFILT_LINES); }
#endif
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2158B_LoadVideofilter(Si2158B,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
    else if (stricmp(entry, "ATVConfig"     )==0)
    {
         Si2158B_setupATVProperties(Si2158B);
         printf("Sending Properties to Si2158B\n");
         Si2158B_downloadATVProperties(Si2158B);
         printf("Done \n");
    }
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2158B_setupDTVProperties(Si2158B);
         printf("Sending Properties to Si2158B\n");
         Si2158B_downloadDTVProperties(Si2158B);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2158B_setupTUNERProperties(Si2158B);
         printf("Sending Properties to Si2158B\n");
         Si2158B_downloadTUNERProperties(Si2158B);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2158B_setupCOMMONProperties(Si2158B);
         printf("Sending Properties to Si2158B\n");
         Si2158B_downloadCOMMONProperties(Si2158B);
         printf("Done \n");
    }
    else if (stricmp(entry, "ATVTune"       )==0)
    {
        ...
        if ((errcode=Si2158B_GetUserFrequency(&freq)) != NO_Si2158B_ERROR) {
           SiTRACE ("Si2158B_GetUserFrequency error 0x%02x\n", errcode);
           return errcode;
        }
        Si2158B_setupProperty(Si2158B,Si2158B_PropertyIndex(Si2158B,"ATV_VIDEO_MODE"));

        if ((errcode=Si2158B_ATVTune (Si2158B, freq,  Si2158B->prop->atv_video_mode.video_sys, Si2158B->prop->atv_video_mode.color, Si2158B->prop->atv_video_mode.invert_spectrum)) != NO_Si2158B_ERROR)
          { printf ("Error returned from ATVTune error = %s\nPlease press <Enter> to continue\n",Si2158B_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}
  ...
  else if (stricmp(entry, "DTVTune"       )==0)
  {
        ...
        if ((errcode=Si2158B_GetUserFrequency(&freq)) != NO_Si2158B_ERROR)
           return errcode;
        Si2158B_setupProperty(Si2158B,Si2158B_PropertyIndex(Si2158B, "DTV_MODE"));

    if ((errcode=Si2158B_DTVTune (Si2158B, freq, Si2158B->prop->dtv_mode.bw, Si2158B->prop->dtv_mode.modulation, Si2158B->prop->dtv_mode.invert_spectrum)) != NO_Si2158B_ERROR)
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2158B_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2158B_TunerStatus(Si2158B);}
    else if (stricmp(entry, "ATVStatus"     )==0) { Si2158B_ATVStatus(Si2158B); }
    else if (stricmp(entry, "DTVStatus"     )==0) { Si2158B_DTVStatus(Si2158B);     }
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2158B_XoutOn(Si2158B);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2158B_XoutOff(Si2158B);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2158B_Powerdown(Si2158B);}
    else if (stricmp(entry, "Standby"           )==0) { Si2158B_Standby(Si2158B);}

    else if (stricmp(entry, "ChannelScanM"  )==0)
  {
     printf("Frequency Scan Start Hz ? ");
    if ((errcode=Si2158B_GetUserFrequency(&minRange)) != NO_Si2158B_ERROR)
       return errcode;

    printf("Frequency Scan Stop Hz ? ");
    if ((errcode=Si2158B_GetUserFrequency(&maxRange)) != NO_Si2158B_ERROR)
       return errcode;

    Si2158B_setupProperty(Si2158B,Si2158B_PropertyIndex(Si2158B,"ATV_RSQ_RSSI_THRESHOLD"));

    Si2158B_ATV_Channel_Scan_M(Si2158B,minRange,maxRange, Si2158B->prop->atv_rsq_rssi_threshold.lo, Si2158B->prop->atv_rsq_rssi_threshold.hi);
    if ((Si2158B->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      for (i=0; i< Si2158B->ChannelListSize;++i)
        printf("%d\t%ld\n",i+1,Si2158B->ChannelList[i]);
    }
    else
    {
  ...
  else if (stricmp(entry, "ChannelScanPal"  )==0)
  {
    ...
    Si2158B_ATV_Channel_Scan_PAL(Si2158B,minRange,maxRange,
                                Si2158B->prop->atv_rsq_rssi_threshold.lo,
                                Si2158B->prop->atv_rsq_rssi_threshold.hi);
    if ((Si2158B->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      printf("Num\tType\tChannel \n");
      for (i=0; i< Si2158B->ChannelListSize;++i)
        printf("%d\t%s\t%ld\n",i,Si2158B->ChannelType[i],Si2158B->ChannelList[i]);
    }
    else
    {
      printf("No Channels Found \n");
   ...
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2158B_AGC_Override(Si2158B, Si2158B_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2158B_AGC_Override(Si2158B, Si2158B_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2158B_AGC_Override(Si2158B, Si2158B_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}
    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2158B_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2158B_help();}
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2158B_L1_API_Init(front_end, Si2158B_BASE_ADDRESS+(2*fe));
      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2158B_configure_i2c_communication  (front_end);
     /* Setup first tuner to use xtal and subsequent tuners will use extclk */
     /* Tuner enbles xout by default */
      if (fe==0)
      {
          front_end->cmd->power_up.clock_mode =  Si2158B_POWER_UP_CMD_CLOCK_MODE_XTAL;
          front_end->cmd->power_up.en_xout    =  Si2158B_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2158B_CONFIG_CLOCKS_CMD_CLOCK_MODE_XTAL;
      }
      else
      {
          front_end->cmd->power_up.clock_mode = Si2158B_POWER_UP_CMD_CLOCK_MODE_EXTCLK;
          front_end->cmd->power_up.en_xout    = Si2158B_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2158B_CONFIG_CLOCKS_CMD_CLOCK_MODE_EXTCLK;
      }
      if (Si2158B_Init(front_end) != 0 ) {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2158B at 0x%02x!\n",Si2158B_BASE_ADDRESS+(2*fe) );
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }
    }
...
#ifdef    Si2158B_COMMAND_LINE_APPLICATION
    while (Si2158B_demoLoop(front_end,""));
#endif /* Si2158B_COMMAND_LINE_APPLICATION */
