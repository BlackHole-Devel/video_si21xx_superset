...
/* define the following token if using >1 tuner and controlling vco settings via the SiLabs superset wrapper */
    ...
    #define Si2151_DEMOD_WRAPPER_VCO
...
int Si2151_PowerUpUsingBroadcastI2C   (L1_Si2151_Context *tuners[], int tuner_count )
{
  ...
  if ( (return_code = Si2151_PowerUpWithPatch(tuners[Si2151_TUNER_BROADCAST_INDEX])) != NO_Si2151_ERROR) {
    SiTRACE("Tuner %d Si2151_PowerUpWithPatch error 0x%02x\n", Si2151_TUNER_BROADCAST_INDEX, return_code);
    return return_code;
  }
...
/************************************************************************************************************************
  ...
  DESCRIPTION: Load firmware from FIRMWARE_TABLE array in Si2151_Firmware_x_y_build_z.h file into Si2151
              Requires Si2151 to be in bootloader mode after PowerUp
  Programming Guide Reference:    Flowchart A.3 (Download FW PATCH flowchart)
...
int  Si2151_GetRF                     (L1_Si2151_Context *api)
{
 ...
 /************************************************************************************************************************
  NAME: Si2151_Tune
  DESCRIPTION: Tune Si2151 in specified mode (ATV/DTV) at center frequency, wait for TUNINT and xTVINT with timeout
...
//SiTRACE("Si2151_L2_VCO_Blocking_PostTune tuner_num %d ==> vco_dest[0] @ 0x%02x,  vco_dest[1] @ 0x%02x,  vco_dest[2] @ 0x%02x\n", tuner_num, tuners[vco_dest[0]]->i2c->address, tuners[vco_dest[1]]->i2c->address, tuners[vco_dest[2]]->i2c->address);
