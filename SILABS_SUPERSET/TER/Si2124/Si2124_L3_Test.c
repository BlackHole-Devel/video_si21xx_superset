 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2124 Layer 3 API


   L3 sample application
   FILE: Si2124_L3_Test.c
   Supported IC : Si2124
   ...
   Date: July 24 2017
****************************************************************************************/
#include "Si2124_L3_Test.h"

/* define TUNER_ONLY if using a Si2124 tuner without demod                                                    */
...
/* define Si2124_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2124_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2124_COMMAND_LINE_APPLICATION */
...
/* If using a Silabs Demod and > 1 tuner then undef both flags and define Si2124_DEMOD_WRAPPER_VCO (see Si2124_L2_API.c) */
#if FRONT_END_COUNT > 1
#define Si2124_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2124_USE_PRETUNE_VCO_BLOCKING_METHOD
#else
#undef Si2124_USE_POSTTUNE_VCO_BLOCKING_METHOD
#undef  Si2124_USE_PRETUNE_VCO_BLOCKING_METHOD
#endif
...
L1_Si2124_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2124_Context         *tuners[FRONT_END_COUNT];
L1_Si2124_Context         *Si2124;
L1_Si2124_Context         *front_end;
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2124_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2124_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2124_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2124_UserInputString function
...
************************************************************************************************************************/
int  Si2124_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2124_trace                          (L1_Si2124_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2124_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2124 EVB
...
/************************************************************************************************************************
  NAME: Si2124_DisplayStatusByte
  DESCRIPTION:Read Si2124 STATUS byte and display status bits
  Parameter:  Pointer to Si2124 Context (I2C address)
  Returns:    Si2124/I2C transaction error code
************************************************************************************************************************/
int  Si2124_DisplayStatusByte              (L1_Si2124_Context *api)
{
    int error_code;

    error_code = Si2124_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2124_GetRevision
  DESCRIPTION:  Execute Si2124 GET_REV function and display response fields
  Parameter:  Pointer to Si2124 Context (I2C address)
...
int  Si2124_GetRevision                    (L1_Si2124_Context *api)
{
    ...
    if (Si2124_L1_GET_REV(api) != NO_Si2124_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2124_SENDING_COMMAND;
    }
    Si2124_L1_GetCommandResponseString(api, Si2124_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2124_TunerStatus
...
int Si2124_TunerStatus (L1_Si2124_Context *Si2124)
{
    ...
    if (Si2124_L1_TUNER_STATUS(Si2124) != NO_Si2124_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2124_SENDING_COMMAND;
    }
  Si2124_L1_GetCommandResponseString(Si2124, Si2124_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2124_GetUserFrequency
...
----------------------------------------------------------------------------\n\
       This is a demonstration application used to illustrate how to use    \n\
            the Si2124 API delivered by Silicon Laboratories                \n\
                                                                            \n\
...
");
  ...
  printf("\
InitAndConfig  : Initialize and Configure the Si2124\n\
Status         : Read Si2124 STATUS\n\
...
************************************************************************************************************************/
int Si2124_demoLoop (L1_Si2124_Context *Si2124, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
    freq = 0;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2124_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2124_trace (Si2124); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2124_Init(Si2124) == 0 ) {
           Si2124_Configure(Si2124);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2124 at 0x%02x!\n", Si2124->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2124_DisplayStatusByte     (Si2124); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2124_GetRevision           (Si2124); }
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2124_LoadVideofilter(Si2124,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2124_setupDTVProperties(Si2124);
         printf("Sending Properties to Si2124\n");
         Si2124_downloadDTVProperties(Si2124);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2124_setupTUNERProperties(Si2124);
         printf("Sending Properties to Si2124\n");
         Si2124_downloadTUNERProperties(Si2124);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2124_setupCOMMONProperties(Si2124);
         printf("Sending Properties to Si2124\n");
         Si2124_downloadCOMMONProperties(Si2124);
         printf("Done \n");
    }
   else if (stricmp(entry, "DTVTune"       )==0)
  {
        printf("DTV Center Frequency (in Hz)? ");
        if ((errcode=Si2124_GetUserFrequency(&freq)) != NO_Si2124_ERROR)
           return errcode;
        Si2124_setupProperty(Si2124,Si2124_PropertyIndex(Si2124, "DTV_MODE"));
    #ifdef Si2124_USE_PRETUNE_VCO_BLOCKING_METHOD
         if ((errcode =  Si2124_L2_VCO_Blocking_PreTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2124_ERROR)
         {
             return errcode;
         }
    #endif // Si2124_USE_PRETUNE_VCO_BLOCKING_METHOD

    errcode=Si2124_DTVTune (Si2124, freq, Si2124->prop->dtv_mode.bw, Si2124->prop->dtv_mode.modulation, Si2124->prop->dtv_mode.invert_spectrum);
    if ((errcode != NO_Si2124_ERROR ) && (errcode != ERROR_Si2124_xTVINT_TIMEOUT))
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2124_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

    #ifdef Si2124_USE_POSTTUNE_VCO_BLOCKING_METHOD
          if ((errcode =  Si2124_L2_VCO_Blocking_PostTune( tuners, fe, FRONT_END_COUNT)) != NO_Si2124_ERROR)
          {
              return errcode;
          }
    #endif // Si2124_USE_POSTTUNE_VCO_BLOCKING_METHOD


  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2124_TunerStatus(Si2124);}
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2124_XoutOn(Si2124);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2124_XoutOff(Si2124);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2124_Powerdown(Si2124);}
    else if (stricmp(entry, "Standby"           )==0) { Si2124_Standby(Si2124);}
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2124_AGC_Override(Si2124, Si2124_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2124_AGC_Override(Si2124, Si2124_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2124_AGC_Override(Si2124, Si2124_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}
    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2124_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2124_help();}
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2124_L1_API_Init(front_end, tuner_addr[fe]);
      if (fe==0)
      {
          front_end->cmd->power_up.clock_mode =  Si2124_POWER_UP_CMD_CLOCK_MODE_XTAL;
          front_end->cmd->power_up.en_xout    =  Si2124_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2124_CONFIG_CLOCKS_CMD_CLOCK_MODE_XTAL;
      }
      else
      {
          front_end->cmd->power_up.clock_mode = Si2124_POWER_UP_CMD_CLOCK_MODE_EXTCLK;
          front_end->cmd->power_up.en_xout    = Si2124_POWER_UP_CMD_EN_XOUT_DIS_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2124_CONFIG_CLOCKS_CMD_CLOCK_MODE_EXTCLK;
      }

      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2124_configure_i2c_communication  (front_end);

    }
 /* If using 2 tuners (or more) startup both tuners using CONFIG_I2C command to broadcast firmware download */
#if FRONT_END_COUNT > 1

      if (Si2124_PowerUpUsingBroadcastI2C(tuners,FRONT_END_COUNT) != NO_Si2124_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2124's!\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }

#else /* use standard init for a single tuner */
      if (Si2124_Init(&(FrontEnd_Table[0])) != NO_Si2124_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2124's!\n");
...
#ifdef    Si2124_COMMAND_LINE_APPLICATION
    while (Si2124_demoLoop(front_end,""));
#endif /* Si2124_COMMAND_LINE_APPLICATION */
