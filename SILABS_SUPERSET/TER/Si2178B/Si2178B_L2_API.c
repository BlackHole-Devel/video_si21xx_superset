...
/************************************************************************************************************************/
/* Si2178B API Specific Includes */
#include "Si2178B_L2_API.h"               /* Include file for this code */
#include "Si2178B_firmware_4_1_build_3.h"       /* firmware compatible with Si2178B-marking */
#include "Si2178B_firmware_0_A_build_15.h"      /* firmware compatible with Si2178B-marking */
#define Si2178B_BYTES_PER_LINE 8
...
#endif
/************************************************************************************************************************
  NAME: Si2178B_Configure
  DESCRIPTION: Setup Si2178B video filters, GPIOs/clocks, Common Properties startup, etc.
...
{
    int return_code;
    return_code = NO_Si2178B_ERROR;

 #ifdef USING_ATV_FILTER
    if ((return_code = Si2178B_LoadVideofilter(api,ATV_VIDFILT_TABLE,ATV_VIDFILT_LINES)) != NO_Si2178B_ERROR)
       return return_code;
  #endif /* USING_ATV_FILTER */
     /* load DTV video filter file */
  #ifdef USING_DLIF_FILTER
     if ((return_code = Si2178B_LoadVideofilter(api,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES)) != NO_Si2178B_ERROR)
       return return_code;
  #endif
   /* Set Properties startup configuration         */
    Si2178B_storePropertiesDefaults (api->propShadow);
    /* Edit the Si2178B_storeUserProperties if you have any properties settings different from the standard defaults */
    Si2178B_storeUserProperties     (api->prop);
    ...
    Si2178B_downloadAllProperties(api);

    return return_code;
}
/************************************************************************************************************************
  NAME: Si2178B_PowerUpWithPatch
  DESCRIPTION: Send Si2178B API PowerUp Command with PowerUp to bootloader,
  ...
  Parameter:  pointer to Si2178B Context
  Returns:    Si2178B/I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
int Si2178B_PowerUpWithPatch    (L1_Si2178B_Context *api)
{
    ...
    if (!(api->load_control & SKIP_POWERUP))
    {
        ...
        if ((return_code = Si2178B_pollForCTS  (api)) != NO_Si2178B_ERROR) {
            SiTRACE ("Si2178B_pollForCTS error 0x%02x\n", return_code);
            return return_code;
        }

        if ((return_code = Si2178B_L1_POWER_UP (api,
                                Si2178B_POWER_UP_CMD_SUBCODE_CODE,
        ...
        {
            SiTRACE ("Si2178B_L1_POWER_UP error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
            return return_code;
        }

        /* Get the Part Info from the chip.   This command is only valid in Bootloader mode */
        if ((return_code = Si2178B_L1_PART_INFO(api)) != NO_Si2178B_ERROR) {
            SiTRACE ("Si2178B_L1_PART_INFO error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
            return return_code;
        }
     ...
     if (!(api->load_control & SKIP_LOADFIRMWARE))
     {
          ...
          {
          /* Load the Firmware */
           if ((return_code = Si2178B_LoadFirmware_16(api, Si2178B_FW_4_1b3, FIRMWARE_LINES_4_1b3)) != NO_Si2178B_ERROR) {
             SiTRACE ("Si2178B_LoadFirmware error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
             return return_code;
           }
          }
          ...
          {
            /* Load the Firmware */
             if ((return_code = Si2178B_LoadFirmware_16(api, Si2178B_FW_0_Ab15, FIRMWARE_LINES_0_Ab15)) != NO_Si2178B_ERROR) {
               SiTRACE ("Si2178B_LoadFirmware error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
               return return_code;
             }
          ...
          else
          {
              ...
              return ERROR_Si2178B_INCOMPATIBLE_PART;
          }
     }
     if (!(api->load_control & SKIP_STARTFIRMWARE))
     {
          /*Start the Firmware */
          if ((return_code = Si2178B_StartFirmware(api)) != NO_Si2178B_ERROR) { /* Start firmware */
              SiTRACE ("Si2178B_StartFirmware error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
              return return_code;
          }

        Si2178B_L1_GET_REV (api);
...
/************************************************************************************************************************
  NAME: Si2178B_PowerUpUsingBroadcastI2C
  ...
  Parameter:  tuners, a pointer to a table of L1 Si2178B Contexts
...
int Si2178B_PowerUpUsingBroadcastI2C    (L1_Si2178B_Context *tuners[], int tuner_count )
{
  ...
  int return_code;
  int t;
  #define Si2178B_TUNER_BROADCAST_INDEX 0
  ...
  for (t = 0; t < tuner_count; t++)
  {
      ...
      /* Si2178B_PowerUpWithPatch will return right after Si2178B_L1_PART_INFO, because SKIP_LOADFIRMWARE and SKIP_STARTFIRMWARE are set */
      if ( (return_code = Si2178B_PowerUpWithPatch(tuners[t])) != NO_Si2178B_ERROR) {
        SiTRACE ("Tuner %d Si2178B_PowerUpWithPatch error 0x%02x\n", return_code);
        return return_code;
      }
       if ( (return_code = Si2178B_L1_CONFIG_I2C(tuners[t], Si2178B_CONFIG_I2C_CMD_SUBCODE_CODE, Si2178B_CONFIG_I2C_CMD_I2C_BROADCAST_ENABLED)) != NO_Si2178B_ERROR )
       {
  ...
  logged_i2c_address = tuners[Si2178B_TUNER_BROADCAST_INDEX]->i2c->address;
  tuners[Si2178B_TUNER_BROADCAST_INDEX]->i2c->address = Si2178B_BROADCAST_ADDRESS;
  /* set the load_control flag to SKIP_POWERUP so the firmware is downloaded and started on all tuners only */
  tuners[Si2178B_TUNER_BROADCAST_INDEX]->load_control = SKIP_POWERUP;

  /* Si2178B_PowerUpWithPatch will now broadcast the tuner fw and return
      when all is completed, because load_fw is now '1'                               */
  if ( (return_code = Si2178B_PowerUpWithPatch(tuners[Si2178B_TUNER_BROADCAST_INDEX])) != NO_Si2178B_ERROR) {
    SiTRACE("Tuner %d Si2178B_PowerUpWithPatch error 0x%02x\n", return_code);
    return return_code;
  }

  /* At this stage, all tuners have received the patch, and have been issued
     'Si2178B_StartFirmware'                                                          */
     if ((return_code = Si2178B_Configure    (tuners[Si2178B_TUNER_BROADCAST_INDEX])) != NO_Si2178B_ERROR)
     {
         return return_code;
     }

  /* Return the broadcast tuner address to its 'normal' value                         */
  tuners[Si2178B_TUNER_BROADCAST_INDEX]->i2c->address = logged_i2c_address;

  for (t = 0; t < tuner_count; t++)
  {
    if ( (return_code = Si2178B_L1_CONFIG_I2C(tuners[t], Si2178B_CONFIG_I2C_CMD_SUBCODE_CODE, Si2178B_CONFIG_I2C_CMD_I2C_BROADCAST_DISABLED)) != NO_Si2178B_ERROR )
     {
      SiTRACE("Tuner %d L1_CONFIG_I2C error 0x%02x\n", t, return_code);
         return return_code;
     }
  ...
  {
    /* Set Properties startup configuration         */
    Si2178B_storePropertiesDefaults (tuners[t]->propShadow);
    /* Edit the Si2178B_storeUserProperties if you have any properties settings different from the standard defaults */
    Si2178B_storeUserProperties     (tuners[t]->prop);
 ...
 /* Check CTS for all tuners */
    if ( (return_code = Si2178B_pollForCTS(tuners[t])) != NO_Si2178B_ERROR ) {
...
/************************************************************************************************************************
  NAME: Si2178B_LoadFirmware_16
  DESCRIPTION: Load firmware from firmware_struct array in Si2178B_Firmware_x_y_build_z.h file into Si2178B
              Requires Si2178B to be in bootloader mode after PowerUp
  Programming Guide Reference:    Flowchart A.3 (Download FW PATCH flowchart)

  Parameter:  Si2178B Context (I2C address)
...
int Si2178B_LoadFirmware_16        (L1_Si2178B_Context *api, firmware_struct fw_table[], int nbLines)
{
    ...
    SiTRACE ("Si2178B_LoadFirmware_16 starting...\n");
    SiTRACE ("Si2178B_LoadFirmware_16 nbLines %d\n", nbLines);
    ...
    for (line = 0; line < nbLines; line++)
    {
		...
		{
            /* send firmware_len bytes (up to 16) to Si2178B */
            if ((return_code = Si2178B_L1_API_Patch(api, fw_table[line].firmware_len, fw_table[line].firmware_table)) != NO_Si2178B_ERROR)
            {
              SiTRACE("Si2178B_LoadFirmware_16 error 0x%02x patching line %d: %s\n", return_code, line, Si2178B_L1_API_ERROR_TEXT(return_code) );
              if (line == 0) {
              SiTRACE("The firmware is incompatible with the part!\n");
              }
          SiTraceConfiguration((char*)"traces resume");
              return ERROR_Si2178B_LOADING_FIRMWARE;
            }
            if (line==3) SiTraceConfiguration("traces suspend");
		}
    }
    SiTraceConfiguration("traces resume");
    SiTRACE ("Si2178B_LoadFirmware_16 complete...\n");
    return NO_Si2178B_ERROR;
}
/************************************************************************************************************************
  NAME: Si2178B_LoadFirmware
*******  Deprecated LoadFirmware Function *******
*******  Please use LoadFirmware_16 replacement *****
  DESCRIPTON: Load firmware from FIRMWARE_TABLE array in Si2178B_Firmware_x_y_build_z.h file into Si2178B
              Requires Si2178B to be in bootloader mode after PowerUp
  Programming Guide Reference:    Flowchart A.3 (Download FW PATCH flowchart)
  ...
  Returns:    Si2178B/I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
int Si2178B_LoadFirmware        (L1_Si2178B_Context *api, unsigned char fw_table[], int nbLines)
{
    int return_code;
    int line;
    return_code = NO_Si2178B_ERROR;

    SiTRACE ("Si2178B_LoadFirmware starting...\n");
    SiTRACE ("Si2178B_LoadFirmware nbLines %d\n", nbLines);

    /* for each line in fw_table */
    for (line = 0; line < nbLines; line++)
    {
        /* send Si2178B_BYTES_PER_LINE fw bytes to Si2178B */
        if ((return_code = Si2178B_L1_API_Patch(api, Si2178B_BYTES_PER_LINE, fw_table + Si2178B_BYTES_PER_LINE*line)) != NO_Si2178B_ERROR)
        {
          SiTRACE("Si2178B_LoadFirmware error 0x%02x patching line %d: %s\n", return_code, line, Si2178B_L1_API_ERROR_TEXT(return_code) );
          if (line == 0) {
          SiTRACE("The firmware is incompatible with the part!\n");
          }
          SiTraceConfiguration((char*)"traces resume");
    ...
    SiTraceConfiguration("traces resume");
    SiTRACE ("Si2178B_LoadFirmware complete...\n");
    return NO_Si2178B_ERROR;
}
/************************************************************************************************************************
  NAME: Si2178B_StartFirmware
  DESCRIPTION: Start Si2178B firmware (put the Si2178B into run mode)
  Parameter:   Si2178B Context (I2C address)
...
int Si2178B_StartFirmware       (L1_Si2178B_Context *api)
{
    ...
    if (Si2178B_L1_EXIT_BOOTLOADER(api, Si2178B_EXIT_BOOTLOADER_CMD_FUNC_TUNER, Si2178B_EXIT_BOOTLOADER_CMD_CTSIEN_OFF) != NO_Si2178B_ERROR)
    {
        return ERROR_Si2178B_STARTING_FIRMWARE;
    }
...
/************************************************************************************************************************
  NAME: Si2178B_Init
  DESCRIPTION:Reset and Initialize Si2178B
  Parameter:  Si2178B Context (I2C address)
  Returns:    I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
...
{
    int return_code;
    SiTRACE("Si2178B_Init starting...\n");

    if ((return_code = Si2178B_PowerUpWithPatch(api)) != NO_Si2178B_ERROR) {   /* PowerUp into bootloader */
        SiTRACE ("Si2178B_PowerUpWithPatch error 0x%02x: %s\n", return_code, Si2178B_L1_API_ERROR_TEXT(return_code) );
        return return_code;
    }
    /* At this point, FW is loaded and started.  */
    Si2178B_Configure(api);
    SiTRACE("Si2178B_Init complete...\n");
    return NO_Si2178B_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2178B_UpdateChannelScanFrequency
...
/* add check for user abort here */
 ...
 /************************************************************************************************************************
  NAME: Si2178B_GetRF
  DESCRIPTIION: Retrieve Si2178B tune freq

  Parameter:  Pointer to Si2178B Context (I2C address)
  Returns  :  frequency (Hz) as an int
************************************************************************************************************************/
int  Si2178B_GetRF             (L1_Si2178B_Context *api)
{
    Si2178B_L1_TUNER_STATUS (api, Si2178B_TUNER_STATUS_CMD_INTACK_OK);
        return api->rsp->tuner_status.freq;
}
 /************************************************************************************************************************
  NAME: Si2178B_Tune
  DESCRIPTIION: Tune Si2178B in specified mode (ATV/DTV) at center frequency, wait for TUNINT and xTVINT with timeout

  Parameter:  Pointer to Si2178B Context (I2C address)
  Parameter:  Mode (ATV or DTV) use Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV or Si2178B_TUNER_TUNE_FREQ_CMD_MODE_DTV constants
 ...
 int  Si2178B_Tune              (L1_Si2178B_Context *api, unsigned char mode, unsigned long freq)
{
    ...
    if (Si2178B_L1_TUNER_TUNE_FREQ (api,
                                   mode,
                                   freq) != NO_Si2178B_ERROR)
    {
    ...
    {
        if ((return_code = Si2178B_L1_CheckStatus(api)) != NO_Si2178B_ERROR)
            return return_code;
    ...
    if (!api->status->tunint) {
      ...
      return ERROR_Si2178B_TUNINT_TIMEOUT;
    }

    /* wait for xTVINT, timeout is 110ms for ATVINT and 10 ms for DTVINT */
    start_time = system_time();
    timeout    = ((mode==Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV) ? 110 : 10);
    while ( (system_time() - start_time) < timeout )
    {
        if ((return_code = Si2178B_L1_CheckStatus(api)) != NO_Si2178B_ERROR)
            return return_code;
        if (mode==Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV)
        {
    ...
    return ERROR_Si2178B_xTVINT_TIMEOUT;
}
 /************************************************************************************************************************
  NAME: Si2178B_ATVTune
  DESCRIPTION:Update ATV_VIDEO_MODE and tune ATV mode at center frequency
  Parameter:  Pointer to Si2178B Context (I2C address)
  Parameter:  frequency (Hz)
  Parameter:  Video system , e.g. use constant Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M for system M
  Parameter:  transport,  e.g. use constant Si2178B_ATV_VIDEO_MODE_PROP_TRANS_TERRESTRIAL for terrestrial
  Parameter:  color , e.g. use constant Si2178B_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC for PAL or NTSC
...
int  Si2178B_ATVTune           (L1_Si2178B_Context *api, unsigned long freq, unsigned char video_sys, unsigned char color, unsigned char invert_signal)
{
    ...
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_VIDEO_MODE_PROP) != NO_Si2178B_ERROR)
    {
      return ERROR_Si2178B_SENDING_COMMAND;
    }

    return Si2178B_Tune (api, Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV, freq);
}
 /************************************************************************************************************************
  NAME: Si2178B_DTVTune
  DESCRIPTION: Update DTV_MODE and tune DTV mode at center frequency
  Parameter:  Pointer to Si2178B Context (I2C address)
  Parameter:  frequency (Hz)
  Parameter:  bandwidth , 6,7 or 8 MHz
  Parameter:  modulation,  e.g. use constant Si2178B_DTV_MODE_PROP_MODULATION_DVBT for DVBT mode
  Parameter:  rsp - commandResp structure to returns tune status info.
  Returns:    I2C transaction error code, 0 if successful
  Programming Guide Reference:    Flowchart A.7 (Tune flowchart)
************************************************************************************************************************/
int  Si2178B_DTVTune           (L1_Si2178B_Context *api, unsigned long freq, unsigned char bw, unsigned char modulation, unsigned char invert_spectrum)
{
    int return_code;
    return_code = NO_Si2178B_ERROR;
    ...
    if (Si2178B_L1_SetProperty2(api, Si2178B_DTV_MODE_PROP) != NO_Si2178B_ERROR)
    {
      return ERROR_Si2178B_SENDING_COMMAND;
    }

    return_code = Si2178B_Tune (api, Si2178B_TUNER_TUNE_FREQ_CMD_MODE_DTV, freq);
...
{
    int return_code=0;
    SiTRACE("Si2178B_XoutOn:  Turning Xout ON\n");

    if ((return_code = Si2178B_L1_CONFIG_CLOCKS(api,
                                              Si2178B_CONFIG_CLOCKS_CMD_SUBCODE_CODE,
                                              api->cmd->config_clocks.clock_mode,
                                              Si2178B_CONFIG_CLOCKS_CMD_EN_XOUT_EN_XOUT)) != NO_Si2178B_ERROR)
    return return_code;

    return NO_Si2178B_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2178B_XoutOff
  Parameter:  Pointer to Si2178B Context (I2C address)
  Returns:    I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
...
{
    int return_code=0;
    SiTRACE("Si2178B_XoutOff:  Turning Xout OFF\n");

    if ((return_code = Si2178B_L1_CONFIG_CLOCKS(api,
                                              Si2178B_CONFIG_CLOCKS_CMD_SUBCODE_CODE,
                                              api->cmd->config_clocks.clock_mode,
                                              Si2178B_CONFIG_CLOCKS_CMD_EN_XOUT_DIS_XOUT)) != NO_Si2178B_ERROR)
    return return_code;

    return NO_Si2178B_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2178B_Standby
  Parameter:  Pointer to Si2178B Context (I2C address)
  Returns:    I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
...
{
    int return_code;
    SiTRACE("Si2178B_Standby: Going to Standby. The part will wake on the next command\n");

    if ((return_code = Si2178B_L1_STANDBY(api, Si2178B_STANDBY_CMD_TYPE_LNA_ON)) != NO_Si2178B_ERROR)
    return return_code;

    return NO_Si2178B_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2178B_Powerdown
  Parameter:  Pointer to Si2178B Context (I2C address)
  Returns:    I2C transaction error code, NO_Si2178B_ERROR if successful
************************************************************************************************************************/
...
{
    int return_code;
    SiTRACE("Si2178B_Powerdown: Powering down the part.  Select InitAndConfig to reload.\n");

    if ((return_code = Si2178B_L1_POWER_DOWN(api)) != NO_Si2178B_ERROR)
    return return_code;

    return NO_Si2178B_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2178B_LoadVideofilter
  DESCRIPTION:        Load video filters from vidfiltTable in Si2178B_write_xTV_video_coeffs.h file into Si2178B
...
int Si2178B_LoadVideofilter     (L1_Si2178B_Context *api,vid_filt_struct vidFiltTable[], int lines)
{
    ...
    {
        /* send up to 16 byte I2C command to Si2178B */
        if (Si2178B_L1_API_Patch(api, vidFiltTable[line].vid_filt_len, vidFiltTable[line].vid_filt_table) != NO_Si2178B_ERROR)
        {
            return ERROR_Si2178B_SENDING_COMMAND;
        }
    }
 ...
 /************************************************************************************************************************
  NAME: Si2178B_ATV_Channel_Scan_M
...
int Si2178B_ATV_Channel_Scan_M (L1_Si2178B_Context *api, unsigned long rangeMinHz, unsigned long rangeMaxHz, int minRSSIdBm, int maxRSSIdBm, int minSNRHalfdB, int maxSNRHalfdB)
{
    ...
    api->prop->atv_video_mode.video_sys = Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_M;    /* M ATV demodulation */
    api->prop->atv_video_mode.color     = Si2178B_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;    /* PAL_NTSC color system */
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_VIDEO_MODE_PROP) != NO_Si2178B_ERROR)
    {
       return ERROR_Si2178B_SENDING_COMMAND;
    }
    ...
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_RSQ_RSSI_THRESHOLD_PROP) != NO_Si2178B_ERROR)
    {
       return ERROR_Si2178B_SENDING_COMMAND;
    }
  ...
  /* configure the RSQ / SNR threshold properties */
    ...
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_RSQ_SNR_THRESHOLD_PROP) != NO_Si2178B_ERROR)
    {
      return ERROR_Si2178B_SENDING_COMMAND;
    }

      /* setup ATV audio property for wide SIF scanning*/
    api->prop->atv_audio_mode.audio_sys    =  Si2178B_ATV_AUDIO_MODE_PROP_AUDIO_SYS_SCAN;
    api->prop->atv_audio_mode.chan_bw =  Si2178B_ATV_AUDIO_MODE_PROP_CHAN_BW_DEFAULT;
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_AUDIO_MODE_PROP) != NO_Si2178B_ERROR)
    {
      return ERROR_Si2178B_SENDING_COMMAND;
    }
   /* setup AFC acquistion range property to 1.5MHz for scanning */
    api->prop->atv_afc_range.range_khz    =  1500;
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_AFC_RANGE_PROP) != NO_Si2178B_ERROR)
    {
       return ERROR_Si2178B_SENDING_COMMAND;
    }
   /* Start Scanning */
    ...
    if (Si2178B_UpdateChannelScanFrequency(freq, CHANNEL_NOT_FOUND))
      break;

    /* Call the Tune command to tune the frequency */
    /* if successful (a station was found) then the return value will be 0. */
    if (!Si2178B_Tune (api, Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV, freq))
    {

            /* Get ATV status */
      if (Si2178B_L1_ATV_STATUS (api, Si2178B_ATV_STATUS_CMD_INTACK_OK))
      {
        return ERROR_Si2178B_SENDING_COMMAND;
      }
      ...
      /* if user requested abort then break from the loop */
      if (Si2178B_UpdateChannelScanFrequency(freq + (api->rsp->atv_status.afc_freq * 1000), api->ChannelListSize+1))
        break;
 ...
 /************************************************************************************************************************
  NAME: Si2178B_ATV_Channel_Scan_PAL
...
int Si2178B_ATV_Channel_Scan_PAL(L1_Si2178B_Context *api, unsigned long rangeMinHz, unsigned long rangeMaxHz, int minRSSIdBm, int maxRSSIdBm, int minSNRHalfdB, int maxSNRHalfdB)
{
  ...
  #define SCAN_INTERVAL 1000000
  #define CHANNEL_NOT_FOUND 0
  char standard = Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_B;
  unsigned long freq;
 ...
 /* configure the RSQ / SNR threshold properties */
   ...
   /* Start Scanning */
    ...
    api->prop->atv_video_mode.video_sys = Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_DK;    /* M ATV demodulation */
    api->prop->atv_video_mode.color     = Si2178B_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;    /* PAL_NTSC color system */
    if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_VIDEO_MODE_PROP) != NO_Si2178B_ERROR)
    {
       return ERROR_Si2178B_SENDING_COMMAND;
    ...
    if (!Si2178B_Tune (api, Si2178B_TUNER_TUNE_FREQ_CMD_MODE_ATV, freq))
    {
      ...
      api->prop->atv_video_mode.video_sys = Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_L;    /* L ATV demodulation */
      api->prop->atv_video_mode.color     = Si2178B_ATV_VIDEO_MODE_PROP_COLOR_SECAM;    /* SECAM color system */
      if (Si2178B_L1_SetProperty2(api, Si2178B_ATV_VIDEO_MODE_PROP) != NO_Si2178B_ERROR)
      {
         return ERROR_Si2178B_SENDING_COMMAND;
      }
...
/*    L1_AcquireSoundStandard(SOUND_PROCESSOR_TIMEOUT , &standard);*/
    if ((standard == Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_B) || (standard == Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_GH))
    {
    ...
    else if (standard == Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_I)
    {
        if (posModulation)
        {
                /* if user requested abort then break from the loop */
    ...
    else if ((standard == Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_L) || (standard == Si2178B_ATV_VIDEO_MODE_PROP_VIDEO_SYS_DK))
    {
        if (posModulation)
        {
    ...
    /* Update the callback to indicate the channel is found */
    /* if user requested abort then break from the loop */
    if (Si2178B_UpdateChannelScanFrequency(api->ChannelList[api->ChannelListSize], api->ChannelListSize+1))
      break;
...
{
    int return_code;
    SiTRACE("Si2178B_AGC_Override: mode = %d\n",mode);
    switch (mode)
    {
        case  Si2178B_FORCE_NORMAL_AGC:
            if ((return_code = Si2178B_L1_AGC_OVERRIDE (api, Si2178B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_DISABLE,Si2178B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_DISABLE)) != NO_Si2178B_ERROR)
                return return_code;
            break;
        case  Si2178B_FORCE_MAX_AGC:
            if ((return_code = Si2178B_L1_AGC_OVERRIDE (api, Si2178B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_ENABLE,Si2178B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_DISABLE)) != NO_Si2178B_ERROR)
                return return_code;
            break;
        case  Si2178B_FORCE_TOP_AGC:
            if ((return_code = Si2178B_L1_AGC_OVERRIDE (api, Si2178B_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_DISABLE,Si2178B_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_ENABLE)) != NO_Si2178B_ERROR)
                return return_code;
            break;
        default:
