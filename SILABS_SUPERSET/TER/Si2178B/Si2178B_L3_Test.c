 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2178B Layer 3 API


   L3 sample application
   FILE: Si2178B_L3_Test.c
...
****************************************************************************************/
#include "Si2178B_L3_Test.h"
/* define TUNER_ONLY if using a Si2178B tuner without demod                                                    */
...
/* define Si2178B_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2178B_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2178B_COMMAND_LINE_APPLICATION */
...
L1_Si2178B_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2178B_Context         *Si2178B;
L1_Si2178B_Context         *front_end;
L1_Si2178B_Context          *tuners[FRONT_END_COUNT];
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2178B_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2178B_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2178B_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2178B_UserInputString function
...
************************************************************************************************************************/
int  Si2178B_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2178B_trace                          (L1_Si2178B_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2178B_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2178B EVB
...
/************************************************************************************************************************
  NAME: Si2178B_DisplayStatusByte
  DESCRIPTION:Read Si2178B STATUS byte and display status bits
  Parameter:  Si2178B Context (I2C address)
  Returns:    Si2178B/I2C transaction error code
************************************************************************************************************************/
int  Si2178B_DisplayStatusByte              (L1_Si2178B_Context *api)
{
    int error_code;

    error_code = Si2178B_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2178B_GetRevision
  DESCRIPTION:  Execute Si2178B GET_REV function and display response fields
  Parameter:  Si2178B Context (I2C address)
...
int  Si2178B_GetRevision                    (L1_Si2178B_Context *api)
{
    ...
    if (Si2178B_L1_GET_REV(api) != NO_Si2178B_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2178B_SENDING_COMMAND;
    }
    Si2178B_L1_GetCommandResponseString(api, Si2178B_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2178B_ATVStatus
...
int Si2178B_ATVStatus (L1_Si2178B_Context *Si2178B)
{
    ...
    if (Si2178B_L1_ATV_STATUS(Si2178B, Si2178B_ATV_STATUS_CMD_INTACK_CLEAR) != NO_Si2178B_ERROR) { /* execute ATV_STATUS and always read response */
        printf("Error reading ATV_STATUS\n");
        return ERROR_Si2178B_SENDING_COMMAND;
    }
   Si2178B_L1_GetCommandResponseString(Si2178B, Si2178B_ATV_STATUS_CMD_CODE, "\n", message);
   printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  Si2178B_DTVStatus function
...
int Si2178B_DTVStatus (L1_Si2178B_Context *Si2178B)
{
    ...
    if (Si2178B_L1_DTV_STATUS(Si2178B, Si2178B_DTV_STATUS_CMD_INTACK_CLEAR) != 0) { /* execute DTV_STATUS and always read response */
        printf("Error reading DTV_STATUS\n");
        return ERROR_Si2178B_SENDING_COMMAND;
    }
  Si2178B_L1_GetCommandResponseString(Si2178B, Si2178B_DTV_STATUS_CMD_CODE,"\n", message);                  /* display results */
  printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2178B_TunerStatus
...
int Si2178B_TunerStatus (L1_Si2178B_Context *Si2178B)
{
    ...
    if (Si2178B_L1_TUNER_STATUS(Si2178B, Si2178B_TUNER_STATUS_CMD_INTACK_CLEAR) != NO_Si2178B_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2178B_SENDING_COMMAND;
    }
  Si2178B_L1_GetCommandResponseString(Si2178B, Si2178B_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2178B_GetUserFrequency
...
----------------------------------------------------------------------------\n\
       This is a demonstration application used to illustrate how to use    \n\
            the Si2178B API delivered by Silicon Laboratories                \n\
                                                                            \n\
...
/************************************************************************************************************************
  NAME: Si2178B_menu
  DESCRIPTION: Si2178B user menu function
...
");
 ...
 printf("\
InitAndConfig  : Initialize and Configure the Si2178B\n\
Status         : Read Si2178B STATUS\n\
...
************************************************************************************************************************/
int Si2178B_demoLoop (L1_Si2178B_Context *Si2178B, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2178B_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"         )==0) { Si2178B_trace            (Si2178B); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2178B_Init(Si2178B) == 0 ) {
           Si2178B_Configure(Si2178B);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2178B at 0x%02x!\n", Si2178B->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2178B_DisplayStatusByte     (Si2178B); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2178B_GetRevision           (Si2178B); }
#ifdef USING_ATV_FILTER
    else if (strcmp_nocase(entry, "LoadATVVidFilt"   )==0) { Si2178B_LoadVideofilter(Si2178B,ATV_VIDFILT_TABLE,ATV_VIDFILT_LINES); }
#endif
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2178B_LoadVideofilter(Si2178B,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
    else if (stricmp(entry, "ATVConfig"     )==0)
    {
         Si2178B_setupATVProperties(Si2178B);
         printf("Sending Properties to Si2178B\n");
         Si2178B_downloadATVProperties(Si2178B);
         printf("Done \n");
    }
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2178B_setupDTVProperties(Si2178B);
         printf("Sending Properties to Si2178B\n");
         Si2178B_downloadDTVProperties(Si2178B);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2178B_setupTUNERProperties(Si2178B);
         printf("Sending Properties to Si2178B\n");
         Si2178B_downloadTUNERProperties(Si2178B);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2178B_setupCOMMONProperties(Si2178B);
         printf("Sending Properties to Si2178B\n");
         Si2178B_downloadCOMMONProperties(Si2178B);
         printf("Done \n");
    }
    else if (stricmp(entry, "ATVTune"       )==0)
    {
        ...
        if ((errcode=Si2178B_GetUserFrequency(&freq)) != NO_Si2178B_ERROR) {
           SiTRACE ("Si2178B_GetUserFrequency error 0x%02x\n", errcode);
           return errcode;
        }
        Si2178B_setupProperty(Si2178B,Si2178B_PropertyIndex(Si2178B,"ATV_VIDEO_MODE"));

        if ((errcode=Si2178B_ATVTune (Si2178B, freq,  Si2178B->prop->atv_video_mode.video_sys, Si2178B->prop->atv_video_mode.color, Si2178B->prop->atv_video_mode.invert_signal)) != NO_Si2178B_ERROR)
          { printf ("Error returned from ATVTune error = %s\nPlease press <Enter> to continue\n",Si2178B_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}
  ...
  else if (stricmp(entry, "DTVTune"       )==0)
  {
        ...
        if ((errcode=Si2178B_GetUserFrequency(&freq)) != NO_Si2178B_ERROR)
           return errcode;
        Si2178B_setupProperty(Si2178B,Si2178B_PropertyIndex(Si2178B, "DTV_MODE"));

    if ((errcode=Si2178B_DTVTune (Si2178B, freq, Si2178B->prop->dtv_mode.bw, Si2178B->prop->dtv_mode.modulation, Si2178B->prop->dtv_mode.invert_spectrum)) != NO_Si2178B_ERROR)
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2178B_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2178B_TunerStatus(Si2178B);}
    else if (stricmp(entry, "ATVStatus"     )==0) { Si2178B_ATVStatus(Si2178B); }
    else if (stricmp(entry, "DTVStatus"     )==0) { Si2178B_DTVStatus(Si2178B);     }
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2178B_XoutOn(Si2178B);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2178B_XoutOff(Si2178B);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2178B_Powerdown(Si2178B);}
    else if (stricmp(entry, "Standby"           )==0) { Si2178B_Standby(Si2178B);}

    else if (stricmp(entry, "ChannelScanM"  )==0)
  {
     printf("Frequency Scan Start Hz ? ");
    if ((errcode=Si2178B_GetUserFrequency(&minRange)) != NO_Si2178B_ERROR)
       return errcode;

    printf("Frequency Scan Stop Hz ? ");
    if ((errcode=Si2178B_GetUserFrequency(&maxRange)) != NO_Si2178B_ERROR)
       return errcode;

    Si2178B_setupProperty(Si2178B,Si2178B_PropertyIndex(Si2178B,"ATV_RSQ_RSSI_THRESHOLD"));
    Si2178B_setupProperty(Si2178B,Si2178B_PropertyIndex(Si2178B,"ATV_RSQ_SNR_THRESHOLD"));

    Si2178B_ATV_Channel_Scan_M(Si2178B,minRange,maxRange, Si2178B->prop->atv_rsq_rssi_threshold.lo, Si2178B->prop->atv_rsq_rssi_threshold.hi,
      Si2178B->prop->atv_rsq_snr_threshold.lo, Si2178B->prop->atv_rsq_snr_threshold.hi );
    if ((Si2178B->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      for (i=0; i< Si2178B->ChannelListSize;++i)
        printf("%d\t%ld\n",i+1,Si2178B->ChannelList[i]);
    }
    else
    {
  ...
  else if (stricmp(entry, "ChannelScanPal"  )==0)
  {
    ...
    Si2178B_ATV_Channel_Scan_PAL(Si2178B,minRange,maxRange,
                                Si2178B->prop->atv_rsq_rssi_threshold.lo,
                                Si2178B->prop->atv_rsq_rssi_threshold.hi,
                                Si2178B->prop->atv_rsq_snr_threshold.lo,
                                Si2178B->prop->atv_rsq_snr_threshold.hi );
    if ((Si2178B->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      printf("Num\tType\tChannel \n");
      for (i=0; i< Si2178B->ChannelListSize;++i)
        printf("%d\t%s\t%ld\n",i,Si2178B->ChannelType[i],Si2178B->ChannelList[i]);
    }
    else
    {
      printf("No Channels Found \n");
   ...
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2178B_AGC_Override(Si2178B, Si2178B_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2178B_AGC_Override(Si2178B, Si2178B_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2178B_AGC_Override(Si2178B, Si2178B_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}

    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2178B_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2178B_help();}
...
int  main(int argc, char *argv[]) {
    ...
    int tuner_addr[] = {Si2178B_BASE_ADDRESS,0xc6};
...
for (fe=0; fe<FRONT_END_COUNT; fe++)
   tuners[fe]= &(FrontEnd_Table[fe]);
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++)
    {
      ...
      Si2178B_L1_API_Init(front_end, tuner_addr[fe]);
      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2178B_configure_i2c_communication  (front_end,demod);

      //use XTAL for 1st tuner and if additional tuners use EXTCLK
      if (fe==0)
      {
          front_end->cmd->power_up.clock_mode =  Si2178B_POWER_UP_CMD_CLOCK_MODE_XTAL;
          front_end->cmd->power_up.en_xout    =  Si2178B_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2178B_CONFIG_CLOCKS_CMD_CLOCK_MODE_XTAL;
      }
      else
      {
          front_end->cmd->power_up.clock_mode = Si2178B_POWER_UP_CMD_CLOCK_MODE_EXTCLK;
          front_end->cmd->power_up.en_xout    = Si2178B_POWER_UP_CMD_EN_XOUT_DIS_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2178B_CONFIG_CLOCKS_CMD_CLOCK_MODE_EXTCLK;
      }
 ...
 /* If using 2 tuners (or more) startup both tuners using CONFIG_I2C command to broadcast firmware download */
      ...
      if (Si2178B_PowerUpUsingBroadcastI2C(tuners,FRONT_END_COUNT) != NO_Si2178B_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2178B's!\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }

#else /* use standard init for a single tuner */
      if (Si2178B_Init(&(FrontEnd_Table[0])) != NO_Si2178B_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2178B's!\n");
...
#ifdef    Si2178B_COMMAND_LINE_APPLICATION
    while (Si2178B_demoLoop(front_end,""));
#endif /* Si2178B_COMMAND_LINE_APPLICATION */
