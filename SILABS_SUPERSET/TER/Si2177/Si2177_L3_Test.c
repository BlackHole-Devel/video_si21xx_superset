 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Silicon Laboratories Broadcast Si2177 Layer 3 API


   L3 sample application
   FILE: Si2177_L3_Test.c
   Supported IC : Si2177
   ...
   Date: June 30 2015
****************************************************************************************/
#include "Si2177_L3_Test.h"

/* define TUNER_ONLY if using a Si2177 tuner without demod                                                    */
...
/* define Si2177_COMMAND_LINE_APPLICATION if using this code in console mode                                                     */
/* (Si2177_COMMAND_LINE_APPLICATION should be defined at project/makefile level to use 100% identical code for console and TV )  */
/* #define Si2177_COMMAND_LINE_APPLICATION */
...
L1_Si2177_Context          FrontEnd_Table   [FRONT_END_COUNT];
L1_Si2177_Context         *tuners[FRONT_END_COUNT];
L1_Si2177_Context         *Si2177;
L1_Si2177_Context         *front_end;
...
#ifdef    SiTRACES
/************************************************************************************************************************
  Si2177_UserInput_traces function
  Use:        send a traces configuration message to the L0 SiTRACES functions
              the Si2177_UserInput_traces function asks the user which type of configuration he wishes.
...
************************************************************************************************************************/
void Si2177_UserInput_traces               (void)
{
    char *configuration;
    char *msg;
...
/************************************************************************************************************************
  Si2177_UserInputString function
...
************************************************************************************************************************/
int  Si2177_UserInputString                (char* text, int max_size) {
  char *newline;
...
************************************************************************************************************************/
void Si2177_trace                          (L1_Si2177_Context *api) {
    int trackFlag;
...
#endif /* TUNER_ONLY */
/************************************************************************************************************************
  NAME: Si2177_configure_i2c_communication
  DESCRIPTION:        Setup USB communication with Si2177 EVB
...
/************************************************************************************************************************
  NAME: Si2177_DisplayStatusByte
  DESCRIPTION:Read Si2177 STATUS byte and display status bits
  Parameter:  Si2177 Context (I2C address)
  Returns:    Si2177/I2C transaction error code
************************************************************************************************************************/
int  Si2177_DisplayStatusByte              (L1_Si2177_Context *api)
{
    int error_code;

    error_code = Si2177_L1_CheckStatus(api);
    if (error_code)
    {
...
/************************************************************************************************************************
  NAME : Si2177_GetRevision
  DESCRIPTION:  Execute Si2177 GET_REV function and display response fields
  Parameter:  Si2177 Context (I2C address)
...
int  Si2177_GetRevision                    (L1_Si2177_Context *api)
{
    ...
    if (Si2177_L1_GET_REV(api) != NO_Si2177_ERROR)          /* execute GET_REV and always read response */
    {
        printf("Error reading GET_REV\n");
        return ERROR_Si2177_SENDING_COMMAND;
    }
    Si2177_L1_GetCommandResponseString(api, Si2177_GET_REV_CMD_CODE,"\n", message);
    printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2177_ATVStatus
...
int Si2177_ATVStatus (L1_Si2177_Context *Si2177)
{
    ...
    if (Si2177_L1_ATV_STATUS(Si2177, Si2177_ATV_STATUS_CMD_INTACK_CLEAR) != NO_Si2177_ERROR) { /* execute ATV_STATUS and always read response */
        printf("Error reading ATV_STATUS\n");
        return ERROR_Si2177_SENDING_COMMAND;
    }
   Si2177_L1_GetCommandResponseString(Si2177, Si2177_ATV_STATUS_CMD_CODE, "\n", message);
   printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  Si2177_DTVStatus function
...
int Si2177_DTVStatus (L1_Si2177_Context *Si2177)
{
    ...
    if (Si2177_L1_DTV_STATUS(Si2177, Si2177_DTV_STATUS_CMD_INTACK_CLEAR) != 0) { /* execute DTV_STATUS and always read response */
        printf("Error reading DTV_STATUS\n");
        return ERROR_Si2177_SENDING_COMMAND;
    }
  Si2177_L1_GetCommandResponseString(Si2177, Si2177_DTV_STATUS_CMD_CODE,"\n", message);                  /* display results */
  printf("%s\n",message);

    return 0;
}
/************************************************************************************************************************
  NAME: Si2177_TunerStatus
...
int Si2177_TunerStatus (L1_Si2177_Context *Si2177)
{
    ...
    if (Si2177_L1_TUNER_STATUS(Si2177, Si2177_TUNER_STATUS_CMD_INTACK_CLEAR) != NO_Si2177_ERROR) { /* execute TUNER_STATUS and always read response */
        printf("Error reading TUNER_STATUS\n");
        return ERROR_Si2177_SENDING_COMMAND;
    }
  Si2177_L1_GetCommandResponseString(Si2177, Si2177_TUNER_STATUS_CMD_CODE, "\n", message);
  printf("%s\n",message);/* display results */

    return 0;
}
/************************************************************************************************************************
  NAME: Si2177_GetUserFrequency
...
----------------------------------------------------------------------------\n\
       This is a demonstration application used to illustrate how to use    \n\
            the Si2177 API delivered by Silicon Laboratories                \n\
                                                                            \n\
...
/************************************************************************************************************************
  NAME: Si2177_menu
  DESCRIPTION: Si2177 user menu function
...
");
  ...
  printf("\
InitAndConfig  : Initialize and Configure the Si2177\n\
Status         : Read Si2177 STATUS\n\
...
************************************************************************************************************************/
int Si2177_demoLoop (L1_Si2177_Context *Si2177, char* choice) {
    char entry[MAX_LENGTH];
    int errcode;
    unsigned long freq;
...
#ifdef    TUNER_ONLY
#ifdef    SiTRACES
    else if (strcmp_nocase(entry, "traces"   )==0) { Si2177_UserInput_traces();}
#endif /* SiTRACES */
    ...
    else if (strcmp_nocase(entry, "trace"    )==0) { Si2177_trace (Si2177); }
#endif /* TUNER_ONLY */
    else if (strcmp_nocase(entry, "InitAndConfig"          )==0)
    {
         if (Si2177_Init(Si2177) == 0 ) {
           Si2177_Configure(Si2177);
         } else {
          SiTRACE("\n");
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("ERROR initializing the Si2177 at 0x%02x!\n", Si2177->i2c->address );
          SiTRACE("ERROR ----------------------------------\n");
          SiTRACE("\n");
         }
    }
    else if (strcmp_nocase(entry, "Status"        )==0) { Si2177_DisplayStatusByte     (Si2177); }
    else if (strcmp_nocase(entry, "GetRevision"   )==0) { Si2177_GetRevision           (Si2177); }
#ifdef USING_ALIF_FILTER
    else if (strcmp_nocase(entry, "LoadALIFVidFilt"   )==0) { Si2177_LoadVideofilter(Si2177,ALIF_VIDFILT_TABLE,ALIF_VIDFILT_LINES); }
#endif
#ifdef USING_DLIF_FILTER
    else if (strcmp_nocase(entry, "LoadDLIFVidFilt"   )==0) { Si2177_LoadVideofilter(Si2177,DLIF_VIDFILT_TABLE,DLIF_VIDFILT_LINES); }
#endif
    else if (stricmp(entry, "ATVConfig"     )==0)
    {
         Si2177_setupATVProperties(Si2177);
         printf("Sending Properties to Si2177\n");
         Si2177_downloadATVProperties(Si2177);
         printf("Done \n");
    }
    else if (stricmp(entry, "DTVConfig"     )==0)
    {
         Si2177_setupDTVProperties(Si2177);
         printf("Sending Properties to Si2177\n");
         Si2177_downloadDTVProperties(Si2177);
         printf("Done \n");
     }
    else if (stricmp(entry, "TunerConfig"   )==0)
    {
         Si2177_setupTUNERProperties(Si2177);
         printf("Sending Properties to Si2177\n");
         Si2177_downloadTUNERProperties(Si2177);
         printf("Done \n");
    }
    else if (stricmp(entry, "CommonConfig"  )==0)
    {
         Si2177_setupCOMMONProperties(Si2177);
         printf("Sending Properties to Si2177\n");
         Si2177_downloadCOMMONProperties(Si2177);
         printf("Done \n");
    }
    else if (stricmp(entry, "ATVTune"       )==0)
    {
        ...
        if ((errcode=Si2177_GetUserFrequency(&freq)) != NO_Si2177_ERROR) {
           SiTRACE ("Si2177_GetUserFrequency error 0x%02x\n", errcode);
           return errcode;
        }
        Si2177_setupProperty(Si2177,Si2177_PropertyIndex(Si2177,"ATV_VIDEO_MODE"));

        if ((errcode=Si2177_ATVTune (Si2177, freq,  Si2177->prop->atv_video_mode.video_sys, Si2177->prop->atv_video_mode.color, Si2177->prop->atv_video_mode.invert_signal)) != NO_Si2177_ERROR)
          { printf ("Error returned from ATVTune error = %s\nPlease press <Enter> to continue\n",Si2177_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}
  ...
  else if (stricmp(entry, "DTVTune"       )==0)
  {
        ...
        if ((errcode=Si2177_GetUserFrequency(&freq)) != NO_Si2177_ERROR)
           return errcode;
        Si2177_setupProperty(Si2177,Si2177_PropertyIndex(Si2177, "DTV_MODE"));

    if ((errcode=Si2177_DTVTune (Si2177, freq, Si2177->prop->dtv_mode.bw, Si2177->prop->dtv_mode.modulation, Si2177->prop->dtv_mode.invert_spectrum)) != NO_Si2177_ERROR)
      { printf ("Error returned from DTVTune error = %s\nPlease press <Enter> to continue\n",Si2177_L1_API_ERROR_TEXT(errcode)); getc(stdin); return 1;}

  }
    else if (stricmp(entry, "TunerStatus"   )==0) { Si2177_TunerStatus(Si2177);}
    else if (stricmp(entry, "ATVStatus"     )==0) { Si2177_ATVStatus(Si2177); }
    else if (stricmp(entry, "DTVStatus"     )==0) { Si2177_DTVStatus(Si2177);     }
    else if (stricmp(entry, "CLS"           )==0) { system("cls");}
    else if (stricmp(entry, "XOUTOn"           )==0) { Si2177_XoutOn(Si2177);}
    else if (stricmp(entry, "XOUTOff"           )==0) { Si2177_XoutOff(Si2177);}
    else if (stricmp(entry, "Powerdown"           )==0) { Si2177_Powerdown(Si2177);}
    else if (stricmp(entry, "Standby"           )==0) { Si2177_Standby(Si2177);}

    else if (stricmp(entry, "ChannelScanM"  )==0)
  {
     printf("Frequency Scan Start Hz ? ");
    if ((errcode=Si2177_GetUserFrequency(&minRange)) != NO_Si2177_ERROR)
       return errcode;

    printf("Frequency Scan Stop Hz ? ");
    if ((errcode=Si2177_GetUserFrequency(&maxRange)) != NO_Si2177_ERROR)
       return errcode;

    Si2177_setupProperty(Si2177,Si2177_PropertyIndex(Si2177,"ATV_RSQ_RSSI_THRESHOLD"));

    Si2177_ATV_Channel_Scan_M(Si2177,minRange,maxRange, Si2177->prop->atv_rsq_rssi_threshold.lo, Si2177->prop->atv_rsq_rssi_threshold.hi,
                             Si2177->prop->atv_rsq_snr_threshold.lo,Si2177->prop->atv_rsq_snr_threshold.hi );
    if ((Si2177->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      for (i=0; i< Si2177->ChannelListSize;++i)
        printf("%d\t%ld\n",i+1,Si2177->ChannelList[i]);
    }
    else
    {
  ...
  else if (stricmp(entry, "ChannelScanPal"  )==0)
  {
    ...
    Si2177_ATV_Channel_Scan_PAL(Si2177,minRange,maxRange,
                                Si2177->prop->atv_rsq_rssi_threshold.lo,
                                Si2177->prop->atv_rsq_rssi_threshold.hi,
                                Si2177->prop->atv_rsq_snr_threshold.lo,
                                Si2177->prop->atv_rsq_snr_threshold.hi );
    if ((Si2177->ChannelListSize) > 0)
    {
      printf("Channels Found \n");
      printf("Num\tType\tChannel \n");
      for (i=0; i< Si2177->ChannelListSize;++i)
        printf("%d\t%s\t%ld\n",i,Si2177->ChannelType[i],Si2177->ChannelList[i]);
    }
    else
    {
      printf("No Channels Found \n");
   ...
   else if (stricmp(entry,"AGCOverride"    )==0)
    {
        ...
        if      ( (strcmp_nocase(entry, "MAX")==0) ) { Si2177_AGC_Override(Si2177, Si2177_FORCE_MAX_AGC); }
        else if ( (strcmp_nocase(entry, "TOP")==0) ) { Si2177_AGC_Override(Si2177, Si2177_FORCE_TOP_AGC); }
        else if ( (strcmp_nocase(entry, "NORMAL")==0) ) { Si2177_AGC_Override(Si2177, Si2177_FORCE_NORMAL_AGC); }
        else    { printf ("Error Invalid Choice\nPlease press <Enter> to continue\n"); getc(stdin); return 1;}
    }
    else if (strcmp_nocase(entry, "m"             )==0) { Si2177_menu(1);}
    else if (strcmp_nocase(entry, "help"          )==0) { Si2177_help();}
...
int  main(int argc, char *argv[]) {
    ...
    int tuner_addr[] = {Si2177_BASE_ADDRESS,0xc6};
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++)
    {
      ...
      Si2177_L1_API_Init(front_end, tuner_addr[fe]);
      front_end->i2c->trackRead = front_end->i2c->trackWrite = 1;
      Si2177_configure_i2c_communication  (front_end,demod);

      //use XTAL for 1st tuner and if additional tuners use EXTCLK
      if (fe==0)
      {
          front_end->cmd->power_up.clock_mode =  Si2177_POWER_UP_CMD_CLOCK_MODE_XTAL;
          front_end->cmd->power_up.en_xout    =  Si2177_POWER_UP_CMD_EN_XOUT_EN_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2177_CONFIG_CLOCKS_CMD_CLOCK_MODE_XTAL;
      }
      else
      {
          front_end->cmd->power_up.clock_mode = Si2177_POWER_UP_CMD_CLOCK_MODE_EXTCLK;
          front_end->cmd->power_up.en_xout    = Si2177_POWER_UP_CMD_EN_XOUT_DIS_XOUT;
          front_end->cmd->config_clocks.clock_mode = Si2177_CONFIG_CLOCKS_CMD_CLOCK_MODE_EXTCLK;
      }
 ...
 /* If using 2 tuners (or more) startup both tuners using CONFIG_I2C command to broadcast firmware download */
      ...
      if (Si2177_PowerUpUsingBroadcastI2C(tuners,FRONT_END_COUNT) != NO_Si2177_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2177's!\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("\n");
      }

#else /* use standard init for a single tuner */
      if (Si2177_Init(&(FrontEnd_Table[0])) != NO_Si2177_ERROR )
      {
        SiTRACE("\n");
        SiTRACE("ERROR ----------------------------------\n");
        SiTRACE("ERROR initializing the Si2177's!\n");
...
#ifdef    Si2177_COMMAND_LINE_APPLICATION
    while (Si2177_demoLoop(front_end,""));
#endif /* Si2177_COMMAND_LINE_APPLICATION */
